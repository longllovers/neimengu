...
class InterProcessFileLock:
    """用一个稳定的旁路文件为目标文件提供跨 Python 进程互斥。"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=50.0,
        help="县界向外缓冲距离（米）；0 表示不外扩",
    )
    parser.add_argument(
        "--cpu-percent",
        type=float,
        default=75.0,
        help="本任务及其全部子进程最高可调用的 CPU 资源百分比",
    )
    parser.add_argument(
        "--gdal-memory-gb",
        type=float,
    ...
    parser.add_argument("--gdal-bin", type=Path, help="包含 gdalwarp 和 gdaladdo 的目录")
    parser.add_argument(
        "--overview-max-factor",
        type=int,
        default=256,
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.buffer_distance_m < 0:
        raise ValueError("--buffer-distance-m 不能小于 0")
    try:
...
def limit_cpu_affinity(cpu_percent: float) -> tuple[int, int]:
    """限制当前进程可运行的 CPU；县级 Python/GDAL 子进程会继承该限制。"""
...
def _update_index_locked(index_path: Path, imagery_dir: Path, mode: str, workers: int) -> None:
    ...
    with connect_index(index_path) as connection:
        ...
        failures: list[str] = []
        if changed:
            # GDAL 数据集读取放入 spawn 子进程，避免多个线程共享 GDAL 驱动状态。
            # SQLite 写入仍只发生在当前进程，杜绝并发写索引文件。
...
def metric_buffer_wgs84(geometry: ogr.Geometry, distance_m: float) -> ogr.Geometry:
    """在县域中心的局部米制投影中缓冲，避免把米误当成经纬度。"""
    if distance_m == 0:
        return geometry.Clone()
    centroid = geometry.Centroid()
    local_srs = spatial_ref(
        f"+proj=aeqd +lat_0={centroid.GetY():.12f} +lon_0={centroid.GetX():.12f} "
        "+datum=WGS84 +units=m +no_defs"
    )
    wgs84 = spatial_ref(4326)
    buffered = geometry.Clone()
    buffered.Transform(osr.CoordinateTransformation(wgs84, local_srs))
    buffered = buffered.Buffer(distance_m)
    buffered.Transform(osr.CoordinateTransformation(local_srs, wgs84))
    if not buffered.IsValid():
        buffered = buffered.MakeValid()
    return buffered


def is_albers(reference: osr.SpatialReference) -> bool:
    """判断坐标系的投影方法是否为 Albers 等积圆锥。"""
    projection = reference.GetAttrValue("PROJECTION") or ""
    return "albers" in projection.lower()


def choose_target_albers(raster_rows: Sequence[tuple[str, str, bytes]]) -> osr.SpatialReference:
    """优先沿用已有 Albers；没有 Albers 输入时使用中国通用 Albers。"""
    for _, candidate_wkt, _ in raster_rows:
        candidate = spatial_ref(candidate_wkt)
        if is_albers(candidate):
            return candidate
    return spatial_ref(CHINA_ALBERS_PROJ)


def query_rasters(
    index_path: Path,
    search_geometry: ogr.Geometry,
    ...
    minx, maxx, miny, maxy = search_geometry.GetEnvelope()
    ...
    for path, crs_wkt, footprint_blob in rows:
        ...
        if footprint.Intersects(search_geometry):
            selected.append((path, crs_wkt, bytes(footprint_blob)))
    return selected
...
def _process_county_locked(
    ...
    buffered_wgs84 = metric_buffer_wgs84(county_wgs84, config.buffer_distance_m)
    raster_rows = query_rasters(config.index_path, buffered_wgs84)
    if not raster_rows:
        LOG.warning("[%s %s] 无相交影像", task.code, task.name)
        return task.code, "no_image"
    target_srs = choose_target_albers(raster_rows)
    coverage_wgs84 = ogr.CreateGeometryFromWkb(raster_rows[0][2])
    for _, _, footprint_wkb in raster_rows[1:]:
        coverage_wgs84 = coverage_wgs84.Union(ogr.CreateGeometryFromWkb(footprint_wkb))
    cutline_geometry = buffered_wgs84.Intersection(coverage_wgs84)
    if cutline_geometry is None or cutline_geometry.IsEmpty():
        LOG.warning("[%s %s] 缓冲范围内没有可裁剪影像", task.code, task.name)
        return task.code, "no_image"
    cutline_geometry.Transform(osr.CoordinateTransformation(spatial_ref(4326), target_srs))
    ...
    with tempfile.TemporaryDirectory(prefix=f"county_{task.code}_", dir=config.temp_root) as temp:
        ...
        LOG.info(
            "[%s %s] 相关影像 %d 幅，正在统一到目标 Albers",
            task.code, task.name, len(paths),
        )
        warp_command = [
            config.gdalwarp,
            "-overwrite",
            ...
            "-cl", "cutline",
            "-cutline_srs", target_srs.ExportToWkt(),
            "-t_srs", target_srs.ExportToWkt(),
            "-crop_to_cutline",
            "-r", config.resampling,
            "-of", "GTiff",
        ]
        ...
        warp_command.extend([*paths, str(partial)])
        try:
...
def run_county_subprocess(task: CountyTask, config: RuntimeConfig) -> tuple[str, str]:
    ...
    if return_code or result_line is None:
        ...
        raise RuntimeError(
            f"[{task.code} {task.name}] 并发裁剪任务失败：\n{detail}"
        )
...
def county_worker_main() -> int:
    """隐藏的子进程入口；任务通过 stdin 传入，结果以单行 JSON 返回。"""
    logging.basicConfig(
        level=logging.INFO,
    ...
    try:
        ...
        config = RuntimeConfig(
            ...
            buffer_distance_m=float(config_data["buffer_distance_m"]),
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        LOG.info(
            ...
            "请求并发=%d，有效并发=%d，"
            "每任务线程=%d，索引并发=%d；GDAL 总内存 %.2f GB（每任务 %d MB）",
        ...
        # 线程只负责调度和收集；GDAL/OGR 县任务在独立 Python 子进程中执行。
