...
DEFAULT_DELIVERY_DIR = Path("02参考真值")

FINAL_FIELDS = ("QXMC", "QXDM", "CKDWLX", "CKDWMJ", "CLDWLX", "CLDWMJ")
FINAL_SCHEMA = {
    "geometry": "Polygon",
    "properties": {
        "QXMC": "str:30",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--delivery-dir", type=Path, default=None, help="交付结果目录，默认 output_root/02参考真值。")
    ...
    parser.add_argument("--mode", choices=("skip", "overwrite"), default="skip", help="skip 跳过已有参考真值；overwrite 重新裁剪并覆盖。")
...
CLASS_CODE_FALLBACK = {1: "107", 2: "124", 3: "146", 0: "900"}
TEXT_CODE_FIELDS = ("ZWDM", "DWMC", "class_id", "作物代码", "CKDWLX", "CLDWLX")
TEXT_NAME_FIELDS = ("ZWMC", "ZZLX", "sample_cls", "作物名称", "作物类型", "地物类型")
...
def reference_code_from_sample(row) -> str:
    code_value = first_row_value(row, TEXT_CODE_FIELDS)
    code = normalized_code(code_value, 10)
    if code and code.isdigit():
        return code
    name = first_row_value(row, TEXT_NAME_FIELDS) or code_value
    if name is not None:
        return crop_code_from_text(name, fill=False)
    if "class" in row.index:
        try:
            class_value = int(float(str(row.get("class")).strip()))
        except (TypeError, ValueError):
            class_value = None
        if class_value in CLASS_CODE_FALLBACK:
            return CLASS_CODE_FALLBACK[class_value]
    return crop_code_from_text(None, fill=False)


def measure_code_from_result(row) -> str:
    code_value = first_row_value(row, TEXT_CODE_FIELDS)
    code = normalized_code(code_value, 10)
    if code and code.isdigit():
        return code
    return crop_code_from_text(first_row_value(row, TEXT_NAME_FIELDS) or code_value, fill=False)
...
def build_tasks(args: argparse.Namespace, common_crs: CRS, result_entries: list[ResultEntry]) -> list[ClipTask]:
    ...
    for sample_path in sample_paths:
        ...
        if args.mode == "skip" and delivery_path.exists():
            print(f"[跳过] 参考真值已存在：{delivery_path}")
            continue
...
def read_result_layer(path: Path, bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    try:
        ...
        if source_crs and target_crs and CRS.from_user_input(source_crs) != CRS.from_user_input(target_crs):
            read_bbox = transform_bounds(bbox_value, target_crs, CRS.from_user_input(source_crs))
    except Exception:
        read_bbox = bbox_value
...
def make_output_row(qxmc: str, qxdm: str, reference_code: str, measure_code: str, geom) -> dict[str, object]:
    area = round(float(geom.area), 2)
    return {
        "QXMC": qxmc,
        "QXDM": qxdm,
        "CKDWLX": reference_code,
        "CKDWMJ": area,
        "CLDWLX": measure_code,
        "CLDWMJ": area,
        "geometry": geom,
...
def clip_samples_to_results(sample: gpd.GeoDataFrame, results: gpd.GeoDataFrame, task: ClipTask) -> gpd.GeoDataFrame:
    ...
    for sample_index, sample_row in sample.iterrows():
        ...
        if spatial_index is not None:
            ...
            for result_index in list(candidate_indices):
                ...
                if result_geom is None:
                    continue
                measure_code = measure_code_from_result(result_row)
                try:
                ...
                for part in iter_polygon_parts(clipped_geom):
                    ...
                    # Initial reference truth mirrors the measured classification result;
                    # reviewers later edit CKDWLX only when field verification disagrees.
                    rows.append(make_output_row(task.qxmc, task.qxdm, measure_code, measure_code, part))
        ...
        if fill_geom is not None and fill_geom.area > task.area_epsilon:
            fill_code = crop_code_from_text(None, fill=True)
            for part in iter_polygon_parts(fill_geom):
                if part.area <= task.area_epsilon:
                    continue
                rows.append(make_output_row(task.qxmc, task.qxdm, fill_code, fill_code, part))
    ...
    for field in ("QXMC", "QXDM", "CKDWLX", "CLDWLX"):
        output[field] = output[field].astype(str)
    for field in ("CKDWMJ", "CLDWMJ"):
