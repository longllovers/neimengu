...
def output_schema(columns: Iterable[str]) -> dict[str, object]:
    properties: dict[str, str] = {}
    for field in columns:
        if field == COUNTY_NAME_FIELD:
            properties[field] = "str:30"
        elif field == COUNTY_CODE_FIELD:
            properties[field] = "str:6"
        elif field in (REVIEWER_FIELD, REVIEW_DATE_FIELD):
            properties[field] = "str:30"
        else:
            match = METRIC_FIELD_PATTERN.match(str(field))
            if match:
                properties[field] = output_metric_schema(match.group(2))
...
def write_boundary(gdf: gpd.GeoDataFrame, output_path: Path, encoding: str) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    schema = output_schema(column for column in gdf.columns if column != "geometry")
...
def populated_output_fields(table: pd.DataFrame, crop_codes: list[str] | None) -> list[str]:
    allowed_columns = [COUNTY_NAME_FIELD, COUNTY_CODE_FIELD]
    if crop_codes is None:
        allowed_columns.extend(
            field for field in table.columns if METRIC_FIELD_PATTERN.match(str(field))
        )
    else:
        for code in crop_codes:
            allowed_columns.extend(metric_field_names(code))
    allowed_columns.extend([REVIEWER_FIELD, REVIEW_DATE_FIELD])

    metric_fields = [field for field in allowed_columns if field in table.columns and METRIC_FIELD_PATTERN.match(str(field))]
    populated_rows = table
    if metric_fields:
        populated_rows = table[table[metric_fields].notna().any(axis=1)]
    return [
        column
        for column in allowed_columns
        if column in populated_rows.columns
        and (
            column == REVIEWER_FIELD
            or (
                populated_rows[column].notna().any()
                and populated_rows.loc[populated_rows[column].notna(), column].astype(str).str.strip().ne("").any()
            )
        )
    ]


def select_output_fields(gdf: gpd.GeoDataFrame, crop_codes: list[str]) -> gpd.GeoDataFrame:
    table = pd.DataFrame(gdf.drop(columns="geometry"))
    fields = populated_output_fields(table, crop_codes)
...
def main() -> int:
    ...
    output = select_output_fields(output, crop_codes)
    updated_path = update_boundary_in_place(output, boundary_shp, args.boundary_output, args.encoding)
