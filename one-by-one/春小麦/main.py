...
def reserve_job(job_name, **values):
    """原子地占用指定任务，不影响另一个独立任务。"""
    with JOBS_LOCK:
        if JOBS[job_name]["running"]:
            return False
...
def run_first_job(source_root, output_root, mode):
    ...
    county_boundary_dir = str(BASE_DIR / "00县边界")
    city_boundary_dir = str(BASE_DIR / "00市边界")
    ...
    print(f"[RUN1] 输出文件夹：{sample_dir}", flush=True)
    print(f"[RUN1] 输出文件夹：{truth_dir}", flush=True)
...
def status_payload():
    ...
    run2_path_root = run2.get("input_root") or run2.get("source_root") or ""
    run1_results = []
...
def page_html(message=""):
    ...
    raw_run2_source_root = run2.get("source_root") or ""
    raw_output_root = run1.get("output_root") or ""
    raw_input_root = run2.get("input_root") or ""
    ...
    result_items = []
    if measure_dir:
        windows_measure_dir = convert_linux_path_to_network_path(measure_dir, raw_input_root or raw_run2_source_root)
        result_items.append(
            "<li><strong>03测量值：</strong></li>" + copyable_item(windows_measure_dir)
        )
    if result_dir:
        windows_result_dir = convert_linux_path_to_network_path(result_dir, raw_input_root or raw_run2_source_root)
        result_items.append(
            "<li><strong>04精度评价：</strong></li>" + copyable_item(windows_result_dir)
        )
    if csv_output:
        windows_csv_output = convert_linux_path_to_network_path(csv_output, raw_input_root or raw_run2_source_root)
        result_items.append(
    ...
    <div class="grid">
        <section class="card">
            ...
            <form method="post" action="/run1">
                <label for="source_root">Source Root <span>路径会自动转换</span></label>
                <input id="source_root" name="source_root" value="{source_root}" placeholder="请输入参考真值分类数据的 Windows 或 Linux 根路径" required>
                <label for="output_root">输入文件夹根路径 <span>在此生成 01生成样本、02参考真值</span></label>
                <input id="output_root" name="output_root" value="{output_root}" placeholder="请输入本次任务的 Windows 或 Linux 根路径" required>
        ...
        <section class="card">
            ...
            <form id="run2-form" method="post" action="/run2">
                <label for="run2_source_root">Source Root <span>测量值分类数据</span></label>
                <input id="run2_source_root" name="run2_source_root" value="{run2_source_root}" placeholder="请输入测量值分类数据的 Windows 或 Linux 根路径" required>
                <label for="input_root">输入文件夹根路径 <span>读取 01、02，生成 03、04</span></label>
                <input id="input_root" name="input_root" value="{input_root}" placeholder="请输入已包含 01、02 文件夹的根路径" required>
...
class ScriptPageHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if parsed.path == "/run1":
            ...
            if busy:
                self.respond_html(page_html("运行-1已经在运行中。"))
                return
        ...
        if parsed.path == "/run2":
            with JOBS_LOCK:
                saved_input_root = JOBS["run2"].get("input_root") or ""
            source_root = (form.get("run2_source_root", [""])[0] or "").strip()
            input_root = (form.get("input_root", [saved_input_root])[0] or saved_input_root).strip()
            if not source_root:
                self.respond_html(page_html("请输入运行-2的 Source Root。"))
                return
            if not input_root:
