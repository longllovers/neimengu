...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="原始参考真值根目录；传入后覆盖 result_root。",
    )
    parser.add_argument(
        "--source_root",
        type=Path,
    ...
    parser.add_argument("sample_dir", type=Path, nargs="?", default=DEFAULT_SAMPLE_DIR, help="样本根目录，递归查找 *_样方.shp；默认 01生成样本。")
    ...
    if args.truth_root is not None:
        args.result_root = args.truth_root
...
CLASS_CODE_FALLBACK = {1: "107", 2: "124", 3: "146", 0: "901"}
...
def build_tasks(args: argparse.Namespace, common_crs: CRS, result_entries: list[ResultEntry]) -> list[ClipTask]:
    ...
    for sample_path in sample_paths:
        ...
        task_outputs = [delivery_path]
        if not args.no_staging:
            task_outputs.append(staging_path)
        output_keys = {str(path.resolve()).casefold() for path in task_outputs}
        if seen_output_paths & output_keys:
            print(f"[跳过] 输出路径重复：{city}-{county}-{unit.code}", file=sys.stderr)
            continue
        seen_output_paths.update(output_keys)
...
def read_result_layer(path: Path, bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    zzlx_field = find_field(
        gdf.columns,
        ("ZWDM", "DWMC", "class_id", "作物代码", "ZWMC", "ZZLX", "zzlx", "作物类型", "地物类型"),
        "ZWDM/ZWMC/ZZLX",
    )
    gdf = gdf[[zzlx_field, "geometry"]].rename(columns={zzlx_field: "ZZLX"}).copy()
    # 本流程只处理春小麦（108），其他作物不参与裁剪。
    gdf = gdf[
        gdf["ZZLX"].apply(lambda value: crop_code_from_text(value, fill=False)) == TARGET_CROP_CODE
    ].copy()
    gdf["geometry"] = gdf.geometry.apply(polygonal_only)
    gdf = gdf[gdf.geometry.notna() & ~gdf.geometry.is_empty].copy()
    return gdf
...
def clip_samples_to_results(sample: gpd.GeoDataFrame, results: gpd.GeoDataFrame, task: ClipTask) -> gpd.GeoDataFrame:
    ...
    for sample_index, sample_row in sample.iterrows():
        ...
        if sample_geom is None:
            continue
        is_target_sample = reference_code_from_sample(sample_row) == TARGET_CROP_CODE
        occupied_geoms = []

        if spatial_index is not None and is_target_sample:
            try:
            ...
            for result_index in list(candidate_indices):
                ...
                for part in iter_polygon_parts(clipped_geom):
                    ...
                    # 参考真值的两个地物类型字段均来自输入 ZWDM。
                    # 当前只保留春小麦 108，其他区域在下方统一补为 901。
