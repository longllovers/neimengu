...
TARGET_METRIC_FIELDS = ("MA108", "IoU108", "LR108", "CR108")
SHP_OUTPUT_FIELDS = (COUNTY_NAME_FIELD, COUNTY_CODE_FIELD, *TARGET_METRIC_FIELDS, REVIEWER_FIELD, REVIEW_DATE_FIELD)
CSV_OUTPUT_FIELDS = (
    COUNTY_NAME_FIELD,
    COUNTY_CODE_FIELD,
...
IGNORED_CODES = {"", "0", "000", "901", "None", "none", "nan", "<NA>"}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="精度评价日期 PJRQ；未传入时自动填写实际运行当天日期。",
    )
...
def compute_county_metrics(
    ...
    # 当前项目为春小麦专项，只评价作物代码 108。
    codes = [TARGET_CROP_CODE]
...
def compute_county_metrics_task(
    county_code: str,
    truth_paths: list[Path],
    measure_paths: list[Path],
    area_crs,
    input_encoding: str | None,
    area_epsilon: float,
) -> tuple[str, dict[str, object], dict[str, dict[str, float | None]]]:
    ...
    return county_code, metrics, details
...
def read_boundary(boundary_shp: Path, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    keep_fields.extend(field for field in (*TARGET_METRIC_FIELDS, REVIEWER_FIELD, REVIEW_DATE_FIELD) if field in boundary.columns)
...
def build_output_boundary(
    ...
    has_metric_value = output[numeric_fields].notna().any(axis=1)
    output.loc[~has_metric_value, [REVIEWER_FIELD, REVIEW_DATE_FIELD]] = ""
    ...
    return output[list(SHP_OUTPUT_FIELDS) + ["geometry"]]
...
def output_schema(crop_codes: list[str]) -> dict[str, object]:
    properties: dict[str, str] = {
        ...
        "MA108": output_metric_schema(TARGET_CROP_CODE),
        "IoU108": output_metric_schema(TARGET_CROP_CODE),
        "LR108": output_metric_schema(TARGET_CROP_CODE),
        "CR108": output_metric_schema(TARGET_CROP_CODE),
        REVIEWER_FIELD: "str:30",
        REVIEW_DATE_FIELD: "str:30",
    }
...
def write_csv(
    ...
    table = pd.DataFrame(gdf.drop(columns="geometry"))[list(CSV_OUTPUT_FIELDS)]
    table = table[table[list(TARGET_METRIC_FIELDS)].notna().any(axis=1)]
...
def main() -> int:
    ...
    if not common_counties:
        print("[提示] 没有需要新增计算的区县，将保留全量县边界并输出空值。")
    county_metrics: dict[str, dict[str, object]] = {}
    concurrency = max(1, args.concurrency)
    if common_counties:
        print(f"[准备] 待计算区县={len(common_counties)}；并发数={concurrency}")
        with concurrent.futures.ProcessPoolExecutor(max_workers=concurrency) as executor:
            future_to_code = {
                executor.submit(
                    compute_county_metrics_task,
                    qxdm,
                    truth_by_code[qxdm],
                ...
                for qxdm in common_counties
            }
            completed_count = 0
            for future in concurrent.futures.as_completed(future_to_code):
                qxdm = future_to_code[future]
                county_code, metrics, details = future.result()
                apply_ma_cap(metrics, details, args.ma_cap_code, args.ma_cap_max)
                county_metrics[county_code] = metrics
                completed_count += 1
                print(f"[计算 {completed_count}/{len(common_counties)}] {qxdm}：{', '.join(metrics.keys())}")

    crop_codes = [TARGET_CROP_CODE]
