...
NETWORK_RE = re.compile(
    r"^(?://|/)?(?P<ip>10\.10\.10\.(?:[1-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))/"
    r"(?P<share>datadisk2|data|新建卷|新加卷)(?P<rest>/.*)?$"
)
SHARE_TO_LINUX = {
...
LINUX_TO_SHARE = {value: key for key, value in SHARE_TO_LINUX.items()}
...
def convert_network_path(path: object) -> str | None:
    ...
    if not text:
        return text
    text = text.replace("\\", "/")
    match = NETWORK_RE.match(text)
    if not match:
        return text
    return SHARE_TO_LINUX[match.group("share")] + (match.group("rest") or "")
...
def convert_linux_path_to_network_path(path: object, source_root: object = "") -> str | None:
    ...
    if not ip:
        return text
    text = text.replace("\\", "/")
    for linux_prefix, share in LINUX_TO_SHARE.items():
        if text == linux_prefix or text.startswith(linux_prefix + "/"):
            relative = text[len(linux_prefix) :]
            return (f"//{ip}/{share}" + relative).replace("/", "\\")
    return text.replace("/", "\\")
