...
def convert_network_path(path):
    """Convert the known Windows shares to their Linux mount points."""
    if path is None:
        return path
    path = str(path).strip()
    if not path:
    ...
    match = re.match(r"^/?/?10\.10\.10\.11/([^/]+)(?=/|$)", path)
    if match and match.group(1) in shares:
        prefix = match.group(0)
        return shares[match.group(1)] + path[len(prefix):]
    return path
...
def get_ip_from_source_root(source_root):
    if source_root is None:
        return ""
    match = re.search(r"10\.10\.10\.11", str(source_root).strip())
...
def convert_linux_path_to_network_path(path, source_root=""):
    ...
    for linux_prefix, windows_prefix in mappings:
