...
GREENHOUSE_CODE = "801"
FINAL_FIELDS = (
    "QXMC", "QXDM", "XZMC", "XZDM", "CUNMC", "CUNDM",
    "TBLXMC", "TBLXDM", "TBEJLMC", "TBEJLDM", "TBMJ", "YXSJ", "BZHU",
)
FINAL_TEXT_FIELDS = (
    "QXMC", "QXDM", "XZMC", "XZDM", "CUNMC", "CUNDM",
    "TBLXMC", "TBLXDM", "TBEJLMC", "TBEJLDM", "YXSJ", "BZHU",
)
FINAL_FLOAT_FIELDS = ("TBMJ",)
FINAL_SCHEMA = {
    "geometry": "Polygon",
    "properties": {
        "QXMC": "str:30",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("overwrite", "skip"), default="overwrite", help="skip 跳过已有测量值；overwrite 重新裁剪并覆盖。")
...
def make_output_row(qxmc: str, qxdm: str, result_row, geom) -> dict[str, object]:
    ...
    return {
        "QXMC": qxmc,
        "QXDM": qxdm,
        "XZMC": non_empty_text(result_row.get("XZMC"))[:30],
        "XZDM": non_empty_text(result_row.get("XZDM"))[:9],
        "CUNMC": non_empty_text(result_row.get("CUNMC"))[:100],
        "CUNDM": non_empty_text(result_row.get("CUNDM"))[:12],
        "TBLXMC": non_empty_text(result_row.get("TBLXMC"))[:16] or "农作物",
        "TBLXDM": crop_code_from_text(result_row.get("TBLXDM"))[:4] or "100",
        "TBEJLMC": non_empty_text(result_row.get("TBEJLMC"))[:16] or "大棚",
        "TBEJLDM": GREENHOUSE_CODE,
        "TBMJ": area,
        "YXSJ": non_empty_text(result_row.get("YXSJ"))[:20],
        "BZHU": non_empty_text(result_row.get("BZHU"))[:20],
        "geometry": geom,
    }
