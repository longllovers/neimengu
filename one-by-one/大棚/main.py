...
def get_ip_from_source_root(source_root):
    ...
    match = re.search(r"10\.10\.10\.\d+", text)
...
def normalize_tab_id(tab_id):
    ...
    if not re.fullmatch(r"[A-Za-z0-9_-]{16,80}", tab_id):
        return ""
    return tab_id


def get_tab_jobs(tab_id, create=True):
    tab_id = normalize_tab_id(tab_id)
    if not tab_id:
        return deepcopy(JOB_TEMPLATE)
    with JOBS_LOCK:
        if create and tab_id not in TAB_JOBS:
            TAB_JOBS[tab_id] = deepcopy(JOB_TEMPLATE)
        return deepcopy(TAB_JOBS.get(tab_id, JOB_TEMPLATE))
...
def append_job_output(tab_id, job_name, text):
    if not text:
        return
    with JOBS_LOCK:
        TAB_JOBS[tab_id][job_name]["output"] += text


def run_command(command, tab_id, job_name):
    command_text = command_for_display(command)
    print(f"[RUN] 开始执行：{command_text}", flush=True)
    append_job_output(tab_id, job_name, f"$ {command_text}\n")
    ...
    if process.stdout is not None:
        for line in process.stdout:
            ...
            append_job_output(tab_id, job_name, line)

    returncode = process.wait()
    print(f"[RUN] 执行结束：returncode={returncode}", flush=True)
    append_job_output(tab_id, job_name, f"[RUN] 执行结束：returncode={returncode}\n")
...
def update_job(tab_id, job_name, **values):
    with JOBS_LOCK:
        TAB_JOBS[tab_id][job_name].update(values)


def run_first_job(tab_id, source_root, output_root, mode):
    ...
    print(f"[RUN1] 输出根路径={converted_output_root}", flush=True)
    ...
    for command in commands:
        ...
        returncode, output = run_command(command, tab_id, "run1")
...
def run_second_job(tab_id, source_root, input_root):
    ...
    print(f"[RUN2] 输入根路径={converted_input_root}", flush=True)
    ...
    for command in commands:
        full_output.append(f"$ {command_for_display(command)}\n")
        returncode, output = run_command(command, tab_id, "run2")
        full_output.append(output)
        if returncode != 0:
            final_returncode = returncode
            break
...
def status_payload(tab_id):
    jobs = get_tab_jobs(tab_id)
    run1 = jobs["run1"]
    run2 = jobs["run2"]
    ...
    return {
        ...
        "inputs": {
            "source_root": run1.get("source_root") or "",
            "output_root": run1.get("output_root") or "",
            "mode": run1.get("mode") or "skip",
            "run2_source_root": run2_source_root,
            "input_root": run2.get("input_root") or run1.get("output_root") or "",
        },
    }


def page_html(message="", tab_id=""):
    jobs = get_tab_jobs(tab_id, create=False)
    run1 = jobs["run1"]
    run2 = jobs["run2"]
...
<main>
    ...
    <div class="grid">
        <section class="card">
            <h2>运行-1</h2>
            <form id="run1-form" method="post" action="/run1">
                <input class="tab-id-input" type="hidden" name="tab_id" value="">
                <label for="source_root">输入原始shp文件夹 <span>路径会自动转换</span></label>
        ...
        <section class="card">
            ...
            <form id="run2-form" method="post" action="/run2">
                <input class="tab-id-input" type="hidden" name="tab_id" value="">
                <label for="run2_source_root">输入原始shp文件夹 <span>可独立填写，运行-1 后自动带入</span></label>
...
<script>
const TAB_ID_KEY = "greenhouse-tab-id";
const pageInstanceId = (crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).slice(2));
let tabId = sessionStorage.getItem(TAB_ID_KEY) || "";
let inputsHydrated = false;
...
function applyTabId() {{
    if (!tabId) tabId = createTabId();
    sessionStorage.setItem(TAB_ID_KEY, tabId);
    document.querySelectorAll(".tab-id-input").forEach((input) => {{
        input.value = tabId;
    }});
}}

// 浏览器“复制标签页”时 sessionStorage 也可能被复制；发现仍存活的同 ID
// 标签页后，为新页面重新生成 ID，确保两边任务状态互不干扰。
if ("BroadcastChannel" in window) {{
    const tabChannel = new BroadcastChannel("greenhouse-tab-identity");
    tabChannel.addEventListener("message", (event) => {{
        const data = event.data || {{}};
        if (data.type === "claim" && data.tabId === tabId && data.instanceId !== pageInstanceId) {{
            tabChannel.postMessage({{type: "owned", tabId, target: data.instanceId}});
        }}
        if (data.type === "owned" && data.tabId === tabId && data.target === pageInstanceId) {{
            tabId = createTabId();
            applyTabId();
            inputsHydrated = false;
            refreshStatus();
        }}
    }});
    applyTabId();
    tabChannel.postMessage({{type: "claim", tabId, instanceId: pageInstanceId}});
}} else {{
    applyTabId();
}}
...
async function refreshStatus() {{
    try {{
        const response = await fetch("/status?tab_id=" + encodeURIComponent(tabId), {{cache: "no-store"}});
        if (!response.ok) return;
        const data = await response.json();
        if (!inputsHydrated && data.inputs) {{
            document.getElementById("source_root").value = data.inputs.source_root || "";
            document.getElementById("output_root").value = data.inputs.output_root || "";
            document.getElementById("mode").value = data.inputs.mode || "skip";
            document.getElementById("run2_source_root").value = data.inputs.run2_source_root || "";
            document.getElementById("input_root").value = data.inputs.input_root || "";
            inputsHydrated = true;
        }}
...
async function submitJob(event, jobName) {{
    event.preventDefault();
    const form = event.currentTarget;
    applyTabId();

    if (jobName === "run1") {{
        document.getElementById("run2_source_root").value =
            document.getElementById("source_root").value;
        document.getElementById("input_root").value =
            document.getElementById("output_root").value;
    }}
    ...
    try {{
        ...
        if (!response.ok || !data.ok) {{
            alert(data.message || "提交运行失败。");
        }}
    }} catch (error) {{
        console.log("提交运行异常:", error);
        alert("提交运行失败，请检查服务是否正常。");
    }} finally {{
        await refreshStatus();
    }}
}}

document.getElementById("run1-form").addEventListener("submit", (event) => submitJob(event, "run1"));
document.getElementById("run2-form").addEventListener("submit", (event) => submitJob(event, "run2"));
...
class ScriptPageHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if parsed.path == "/status":
            tab_id = normalize_tab_id(parse_qs(parsed.query).get("tab_id", [""])[0])
            if not tab_id:
                self.respond_json({"ok": False, "message": "缺少有效的标签页标识。"}, status=400)
                return
            self.respond_json(status_payload(tab_id))
            return
    ...
    def do_POST(self):
        ...
        form = parse_qs(data)
        tab_id = normalize_tab_id(form.get("tab_id", [""])[0])

        if parsed.path in ("/run1", "/run2") and not tab_id:
            self.respond_json({"ok": False, "message": "缺少有效的标签页标识，请刷新页面后重试。"}, status=400)
            return
        if tab_id:
            get_tab_jobs(tab_id)
        ...
        if parsed.path == "/run1":
            with JOBS_LOCK:
                running = TAB_JOBS[tab_id]["run1"]["running"]
            if running:
                self.respond_json({"ok": False, "message": "第一步已经在运行中。"}, status=409)
                return
            ...
            print(f"[RUN1] 页面提交：source_root={source_root}, output_root={output_root}, mode={mode}", flush=True)
            if not source_root:
                print("[RUN1] 未输入 --source_root", flush=True)
                self.respond_json({"ok": False, "message": "请输入 --source_root。"}, status=400)
                return
            if not output_root:
                print("[RUN1] 未输入输出文件夹根路径", flush=True)
                self.respond_json({"ok": False, "message": "请输入输出文件夹根路径。"}, status=400)
                return
            if not mode:
                mode = "skip"
            update_job(tab_id, "run1", source_root=source_root, output_root=output_root, mode=mode)
            update_job(tab_id, "run2", source_root=source_root, input_root=output_root)
            start_background(run_first_job, tab_id, source_root, output_root, mode)
            self.respond_json({"ok": True})
            return

        if parsed.path == "/run2":
            with JOBS_LOCK:
                running = TAB_JOBS[tab_id]["run2"]["running"]
            print(f"[RUN2] 页面提交：running={running}", flush=True)
            if running:
                self.respond_json({"ok": False, "message": "第二步已经在运行中。"}, status=409)
                return
            with JOBS_LOCK:
                jobs = TAB_JOBS[tab_id]
            ...
            if not source_root:
                self.respond_json({"ok": False, "message": "请输入运行-2的source_root。"}, status=400)
                return
            if not input_root:
                self.respond_json({"ok": False, "message": "请输入运行-2的input_root。"}, status=400)
                return
            update_job(tab_id, "run2", source_root=source_root, input_root=input_root)
            start_background(run_second_job, tab_id, source_root, input_root)
            self.respond_json({"ok": True})
            return
