...
def cleanup_temp_output_dir(temp_dir):
    """只允许删除本程序在 TEMP_OUTPUT_ROOT 下创建的单次运行目录。"""
...
<main class="page">
    ...
    <section class="layout">
        <form class="card run-form" method="post" data-status-id="pipeline_status" onsubmit="return false;">
            ...
            <h2>投票并合并</h2>
            <p class="hint">先运行 vote.py；成功后自动使用投票输出目录运行 merge_geodata.py。</p>

            <div class="field">
                <input id="df_path" type="text" name="df_path"
                       value="{html_escape(values["df_path"])}"
                       placeholder="{html_escape(defaults["df_path"])}">
                <label class="checkbox-row" for="refresh_shp_cache">
                    <input id="refresh_shp_cache" type="checkbox" name="refresh_shp_cache" value="1">
                    强制刷新输入shp
                </label>
            </div>
            ...
            <div class="field">
                <label for="output2_path">最终输出shp路径 <span>默认：{html_escape(defaults["output2_path"])}</span></label>
            ...
            <div class="actions">
                <button class="btn-vote" type="submit">运行投票并合并</button>
            </div>
                ...
                while (true) {{
                    ...
                    lines.forEach(function(line) {{
                        ...
                        if (eventData.type === "start") {{
                            ...
                            badgeEl.className = "badge";
                            if (statusEl) {{
                                statusEl.textContent = "正在运行：" + eventData.name;
                                statusEl.className = "run-status running";
                            }}
                            if (hasOutput) {{
...
class ScriptRunHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if action == "run_all":
            ...
            if form_data.get("refresh_shp_cache", [""])[0] == "1":
                vote_command.append("--refresh-shp-cache")
        ...
        self.end_headers()
        # 每次请求内严格串行执行两个脚本；不同请求之间最多并行 3 个任务。
        try:
            with TASK_SEMAPHORE:
                vote_returncode = run_command_stream(self, "vote.py", vote_command)
                if vote_returncode == 0:
                    run_command_stream(self, "merge_geodata.py", merge_command)
        finally:
            try:
                cleanup_temp_output_dir(temp_output_dir)
                print(f"[临时目录已删除] {temp_output_dir}", flush=True)
            except Exception as exc:
                print(f"[临时目录删除失败] {temp_output_dir}: {exc}", flush=True)
