...
def extraction_heartbeat(
    ...
    while not stop_event.wait(HEARTBEAT_SECONDS):
        output_size = staging_path.stat().st_size if staging_path.exists() else 0
        output_text = format_size(output_size) if output_size else "尚未开始写入 TIFF"
        print(
            f"{progress_label} 正在处理：{zip_path.name} | "
            f"已用时 {elapsed_text(started_at)} | 临时输出 {output_text}",
            flush=True,
        )
...
def extract_for_cities(
    ...
    if overwrite or reusable is None:
        ...
        heartbeat = threading.Thread(
            target=extraction_heartbeat,
            args=(stop_event, progress_label, zip_path, staging_path, started_at),
            name="zip-extraction-heartbeat",
            daemon=True,
        )
        print(
...
def main() -> int:
    ...
    if not plans:
        if args.dry_run:
            print(
                ...
                f"计划移动 {len(skipped_archives)} 个到 {skipped_root}；"
                "未写入任何文件。"
            )
            return 0
    ...
    for index, (zip_path, city_names, group) in enumerate(plans, 1):
        ...
        try:
            ...
            print(
                f"{progress_label} 处理完成：{zip_path.name} -> "
                f"{len(outputs)} 个城市目录 | "
                f"已用时 {elapsed_text(item_started)}",
                flush=True,
            )
            ...
            print(
                f"{progress_label} 抽取失败：{zip_path} - {exc} | "
                f"已用时 {elapsed_text(item_started)}",
                flush=True,
            )
