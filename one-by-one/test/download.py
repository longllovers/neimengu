    ...
    def download_product(
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                task_id = (
                    progress.add_task(f"{scene_label}{file_id}", total=total_size or None)
                    if progress
                    else None
                )
                downloaded_size = 0
                ...
                console.print(
                    f"[green]{scene_label}✅ 下载成功，开始抽取 10m tif：{file_id}[/green]"
                )
...
class AccountPool:
    ...
    def __init__(self, accounts, max_active_accounts=MAX_ACTIVE_ACCOUNTS):
        self.clients = [
            CDSEClient(
    ...
    def batch_download(
        ...
        if requested_workers > planned_workers:
            console.print(
                f"[bold yellow]⚠️ 请求 {requested_workers} 个线程，但配置最多同时使用 "
                f"{self.max_active_accounts} 个账号；已调整为 {planned_workers} 个线程[/bold yellow]"
            )
        if account_count < planned_accounts:
            console.print(
                f"[bold yellow]⚠️ 计划使用 {planned_accounts} 个账号，但只有 "
                f"{account_count} 个账号可用；已调整为 {actual_workers} 个线程[/bold yellow]"
            )
        console.print(f"[bold green]🚀 开始批量下载 {len(products)} 景数据[/bold green]")
        console.print(f"[bold blue]📁 临时 zip 目录：{temp_output_dir}[/bold blue]")
        console.print(f"[bold blue]📁 tif 输出目录：{final_output_dir}[/bold blue]")
        console.print(
            f"[bold blue]👥 并发策略：请求 {requested_workers} 个线程，实际 "
            f"{actual_workers} 个线程，启用 {account_count} 个账号[/bold blue]"
        )
        for client, worker_limit in zip(active_clients, worker_limits):
            console.print(
                f"[blue]  账号 {client.account_id}：{worker_limit} 个并发线程[/blue]"
            )
        ...
        with progress:
            ...
            for index, item in enumerate(products, 1):
                ...
                if product_id:
                    ...
                    pending.append((index, product_id, display_name, client_index))

            assigned_counts = [0] * account_count
            for _, _, _, client_index in pending:
            ...
            while pending:
                ...
                try:
                    futures = {}
                    for index, product_id, display_name, client_index in pending:
                        ...
                        futures[future] = (index, product_id, display_name, client_index)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="期望并发线程数；每个账号最多 3 个线程，账号不足时自动降低并发",
    )
