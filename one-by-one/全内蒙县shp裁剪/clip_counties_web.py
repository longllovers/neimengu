...
class JobController:
    ...
    def stop(self) -> tuple[bool, str]:
        ...
        self.broker.publish({
            "kind": "terminal",
            "line": f"正在停止裁剪任务及其全部子进程（PID {process.pid}）…",
            "level": "warning",
        })
        process_group: int | None = None
        try:
            if os.name == "nt":
                # Windows 没有可靠的进程组 SIGTERM；直接强制结束整棵进程树。
        ...
        return True, "终止信号已发送，正在确认所有裁剪进程退出。"
    ...
    def _ensure_stopped(
        ...
        try:
            ...
            if os.name != "nt" and process_group is not None:
                # 即使 Python 主进程已先退出，也要清掉仍留在原进程组中的子进程。
                try:
            ...
            try:
                ...
                self.broker.publish({
                    "kind": "terminal",
                    "line": f"进程 PID {process.pid} 未按时退出，请检查系统进程。",
                    "level": "error",
                })
...
def convert_network_path(path: str | Path | None) -> str | None:
    """把 10.10.10.* 的 Windows UNC 共享路径转换为 Linux 挂载路径。"""
    if path is None:
        return None
    ...
    match = re.match(r"^(?://|/)?(10\.10\.10\.\d+)/([^/]+)(/.*)?$", normalized)
    if not match:
        return normalized
    share = match.group(2)
    remainder = match.group(3) or ""
    for share_name, linux_root in mappings:
        if share == share_name:
            return linux_root + remainder
    return normalized
...
badge("正在停止",true);byId("stage").textContent="正在立即终止全部裁剪进程";
...
byId("stage").textContent="正在终止裁剪进程组 PID "+p.pid}}
...
byId("stage").textContent=stopped?"任务已停止，所有裁剪进程均已退出":"脚本已退出，退出码 "+p.exit_code;
