...
TRUTH_CODE_FIELD = "CLTBLXDM"
MEASURE_CODE_FIELD = "TBLXDM"
COUNTY_NAME_FIELD = "QXMC"
COUNTY_CODE_FIELD = "QXDM"
REVIEWER_FIELD = "PJR"
REVIEW_DATE_FIELD = "PJRQ"
IGNORED_CODES = {"", "None", "none", "nan", "<NA>"}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="按 CLTBLXDM 真值和 TBLXDM 测量值计算 MA/IoU/LR/CR，写回新边界 shp 并导出 CSV。"
    )
...
def csv_rate_field_names(code: str) -> tuple[str, str]:
    code = crop_code(code)
    return f"LR{code}", f"CR{code}"
...
def infer_area_crs(truth_by_code: dict[str, list[Path]], measure_by_code: dict[str, list[Path]], fallback_crs):
    for groups in (truth_by_code, measure_by_code):
        for paths in groups.values():
            for path in paths:
                try:
                    gdf = read_vector(path, None)
                except Exception as exc:
                    print(f"[04] 跳过 CRS 检查失败文件：{path}；原因：{exc}", file=sys.stderr)
                    continue
                if gdf.crs is not None and getattr(gdf.crs, "is_projected", False):
                    print(f"[04] 面积计算 CRS 使用样本文件：{path}；CRS={gdf.crs}", flush=True)
                    return gdf.crs
    print(f"[04] 未找到投影样本 CRS，面积计算 CRS 使用边界 CRS：{fallback_crs}", flush=True)
    return fallback_crs
...
def read_layers(paths: list[Path], required_field: str, target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    print(f"[04] 读取图层组：字段={required_field}，文件数={len(paths)}", flush=True)
...
## reference_geom  是真值 s-b

def compute_leakage_rate(reference_geom, measure_geom) -> float | None:
    if reference_geom is None and measure_geom is None:
        return None
    if reference_geom is None or measure_geom is None:
        return 100.0
    ...
    try:
        reference_area = float(reference_geom.area)
        intersection_area = float(reference_geom.intersection(measure_geom).area)
    except Exception:
        reference_geom = make_valid_geom(reference_geom)
        measure_geom = make_valid_geom(measure_geom)
    ...
    if reference_area <= 0:
        return None
    result = ((reference_area - intersection_area) / reference_area) * 100
    return max(0.0, min(100.0, result))



# 计算CR(错检率)
def compute_checking_rate(reference_geom, measure_geom) -> float | None:
    if reference_geom is None and measure_geom is None:
        return None
    if reference_geom is None or measure_geom is None:
        return 100.0
    ...
    try:
        measure_area = float(measure_geom.area)
        intersection_area = float(reference_geom.intersection(measure_geom).area)
    except Exception:
        reference_geom = make_valid_geom(reference_geom)
        measure_geom = make_valid_geom(measure_geom)
    ...
    if measure_area <= 0:
        return None
    result = ((measure_area - intersection_area) / measure_area) * 100
    return max(0.0, min(100.0, result))
...
def compute_county_metrics(
    ...
    print(f"[04] 开始计算区县：{county_code}；真值文件数={len(truth_paths)}；测量文件数={len(measure_paths)}", flush=True)
    ...
    print(f"[04] 区县 {county_code} 读取完成：truth_features={len(truth)}，measure_features={len(measure)}", flush=True)
    if truth.empty:
    ...
    print(f"[04] 区县 {county_code} 参与评价作物代码：{codes}", flush=True)
    ...
    for code in codes:
        ...
        lr_value = compute_leakage_rate(reference_geom, measure_geom)
        cr_value = compute_checking_rate(reference_geom, measure_geom)
        metrics[ma_field] = ma_value
        metrics[iou_field] = iou_value
        metrics[lr_field] = lr_value
        metrics[cr_field] = cr_value
        inter_area = None
        union_area = None
...
def write_csv(
    ...
    if county_metrics is not None and crop_codes is not None:
        for code in crop_codes:
            lr_field, cr_field = csv_rate_field_names(code)
            table[lr_field] = table[COUNTY_CODE_FIELD].apply(
                lambda qxdm: round_metric(county_metrics.get(non_empty_text(qxdm), {}).get(lr_field))
            )
            table[cr_field] = table[COUNTY_CODE_FIELD].apply(
                lambda qxdm: round_metric(county_metrics.get(non_empty_text(qxdm), {}).get(cr_field))
            )
    metric_fields = [
        field
        for field in table.columns
        if METRIC_FIELD_PATTERN.match(str(field)) or str(field).startswith(("LR", "CR"))
    ]
    if metric_fields:
...
def main() -> int:
    args = parse_args()
    print(f"[04] truth_root={args.truth_root}；measure_root={args.measure_root}；boundary={args.boundary_shp}", flush=True)
    print(f"[04] boundary_output={args.boundary_output}；csv_output={args.csv_output}；mode={args.mode}", flush=True)
    ...
    print(f"[04] 边界读取完成：features={len(boundary)}，CRS={boundary.crs}", flush=True)
    truth_by_code = collect_by_county(args.truth_root)
    measure_by_code = collect_by_county(args.measure_root)
    print(f"[04] 收集真值区县数={len(truth_by_code)}；测量区县数={len(measure_by_code)}", flush=True)
    area_crs = infer_area_crs(truth_by_code, measure_by_code, boundary.crs)

    common_counties = sorted(set(truth_by_code) & set(measure_by_code))
    print(f"[04] 可配对区县数={len(common_counties)}", flush=True)
    if args.mode == "skip":
    ...
    for index, qxdm in enumerate(common_counties, start=1):
        ...
        for code in details:
            all_codes.add(code)
            detail_rows.append({COUNTY_CODE_FIELD: qxdm, "TBLXDM": code, **details[code]})
        print(f"[计算 {index}/{len(common_counties)}] {qxdm}：{', '.join(metrics.keys())}")

    existing_codes = metric_codes_from_columns(boundary.columns)
    if args.mode == "skip":
        crop_codes = sorted(existing_codes | all_codes, key=lambda value: (len(str(value)), str(value)))
    else:
        crop_codes = sorted(all_codes, key=lambda value: (len(str(value)), str(value)))
    crop_codes = sorted((set(crop_codes) | REQUIRED_OUTPUT_CODES) - EXCLUDED_OUTPUT_CODES, key=lambda value: (len(str(value)), str(value)))
    print(f"[04] 最终输出作物代码={crop_codes}", flush=True)
