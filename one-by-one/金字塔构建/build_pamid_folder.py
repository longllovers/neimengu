...
# 可以直接在这里填写路径；命令行参数优先。
DEFAULT_TIF_DIR = r"/media/cangling/nas_folder/县级影像/2m挑出影像/按县组织影像/150425克什克腾旗/150425"
...
def convert_network_path(path: str | None) -> str | None:
    """把局域网 Windows UNC 路径转换成当前 Linux 主机的挂载路径。"""
    if path is None:
        return None
    ...
    # UNC 路径可能由网页或 Windows 客户端传入，先统一使用正斜杠。
    ...
    for host_index in range(1, 256):
        for share_name, linux_prefix in share_mounts.items():
            # 同时兼容 //host/share、/host/share 和 host/share 三种写法。
            for windows_prefix in (
                f"//169.254.51.{host_index}/{share_name}",
                f"/169.254.51.{host_index}/{share_name}",
                f"169.254.51.{host_index}/{share_name}",
            ):
