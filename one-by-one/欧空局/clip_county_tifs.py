...
def clip_one_county(
    ...
    def remove_stale_output() -> None:
        ...
        for stale_path in (
            ...
            Path(f"{output_path}.msk"),
        ):
            if stale_path.exists():
                stale_path.unlink()
        for stale_directory in (output_path.parent, output_path.parent.parent):
            try:
                stale_directory.rmdir()
            except OSError:
                pass

    # 先在县级输出根目录写临时 TIFF；确认有有效像元后才创建县区目录。
    temporary_root = output_path.parents[2]
    temporary_root.mkdir(parents=True, exist_ok=True)
    temporary_path = temporary_root / f".{output_path.stem}.tmp.tif"
    ...
    try:
        with rasterio.open(source_path) as source:
            ...
            try:
                ...
                remove_stale_output()
                console.print(
                    f"[yellow]⚠️ {county_name} 与市级有效影像范围不相交，"
                    "不生成县级 TIFF[/yellow]"
                )
                return "no_coverage"
        ...
        if not has_valid_data:
            remove_stale_output()
            console.print(
                f"[yellow]⚠️ {county_name} 在本期市级影像中没有有效像元，"
                "不生成县级 TIFF[/yellow]"
            )
            return "no_coverage"
...
def main() -> int:
        ...
        for source_path in source_files:
            ...
            for _, county in selected.iterrows():
                ...
                if output_path.exists() and not args.overwrite:
                    if not args.dry_run and not output_path.with_suffix(".jpeg").exists():
                        try:
                            create_tif_thumbnail(output_path)
                        except Exception as exc:
                            if "没有有效影像像元" in str(exc):
                                no_coverage += 1
                                console.print(
                                    f"[yellow]⚠️ 已有 TIFF 没有有效影像："
                                    f"{output_path}[/yellow]"
                                )
                            else:
                                failed += 1
                                console.print(
                                    f"[red]❌ 已有 TIFF 的缩略图生成失败："
    ...
    def record_county_result(job: dict, status: str | None, error: Exception | None) -> None:
        nonlocal created, skipped, no_coverage, failed, completed_jobs
        if error is None:
            if status == "created":
                created += 1
                console.print(
                ...
                no_coverage += 1
                console.print(
                    f"[yellow]⚠️ {job['city_name']}/{job['county_name']}："
                    "本期无有效影像，未生成 TIFF[/yellow]"
                )
        else:
            failed += 1
            console.print(
    ...
    console.print(
        f"\n[bold]处理结束：新建 {created}，跳过 {skipped}，"
        f"无影像 {no_coverage}，失败 {failed}[/bold]"
    )
