...
def split_shapefile(
    ...
    overwrite = config.get("overwrite") is True or str(config.get("overwrite", "")).strip().lower() in {"1", "true", "yes", "on"}
    try:
        geometry_concurrency = int(config.get("concurrency", GEOMETRY_CONCURRENCY))
    except (TypeError, ValueError) as exc:
        raise ValueError("空间相交并发数必须是整数。") from exc
    if not 1 <= geometry_concurrency <= 64:
        raise ValueError("空间相交并发数必须在 1 到 64 之间。")
                        ...
                        for county, crop, clipped_feature in results:
                            ...
                            if path is None:
                                ...
                                if path.exists() and not overwrite:
                                    ...
                                    task_state.log(f"保留已有结果，跳过：{path}")
    ...
    if skipped_groups:
        task_state.log(f"已保留并跳过 {len(skipped_groups)} 个已有 shp；启用覆盖选项后可重新生成。")
    if unmatched:
