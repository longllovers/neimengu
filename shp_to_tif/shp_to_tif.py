...
def merge_shapefiles(
    ...
    log(
        f"合并准备：使用 {worker_count} 个独立并发数并行读取和转换 "
...
def run_processing(options: ProcessingOptions, log: Callable[[str], None] = print) -> None:
    ...
    log(f"SHP 并行并发数设置：{options.shp_threads}；TIF 线程固定：{TIF_THREADS}")
    log("=" * 58)
    ...
    if options.tif_only:
        ...
        log(
            f"找到 {len(source_files)} 个 Shapefile，使用 {worker_count} 个独立并发数；"
            f"每个并发数处理一个 SHP，统一设置 class={options.class_value}……"
        )
        ...
        # Fiona/GDAL 的 Shapefile 写入在单个 Python 并发数内受 GIL/驱动锁影响，
        # 使用 spawn 多并发数才能真正并行占用多个 CPU 核；也避免从 HTTP 工作线程 fork。
        with ProcessPoolExecutor(
            max_workers=worker_count,
            mp_context=get_context("spawn"),
        ) as executor:
