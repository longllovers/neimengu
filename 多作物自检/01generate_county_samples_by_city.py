...
DEFAULT_CROP_CODE_PATH = BASE_DIR / "zuowucode.json"
DEFAULT_CONCURRENCY = max(1, min((os.cpu_count() or 1), 2))
...
def normalize_crop_code(value: object) -> str:
...
def load_crop_code_mapping(path: Path) -> dict[str, str]:
    path = Path(path)
    ...
    for name, code in data.items():
        crop_name = normalize_mapping_text(name)
        crop_code = normalize_crop_code(code)
        if crop_name and crop_code:
            mapping[crop_name] = crop_code
    if not mapping:
        raise ValueError(f"作物代码 JSON 没有有效作物代码: {path}")
    return mapping


def crop_name_by_code(crop_code_mapping: dict[str, str]) -> dict[str, str]:
    return {code: name for name, code in crop_code_mapping.items()}


def normalize_crop_code_column(
    gdf: gpd.GeoDataFrame,
    crop_code_mapping: dict[str, str],
    class_mapping: list[dict[str, object]] | None = None,
) -> gpd.GeoDataFrame:
    gdf = apply_flexible_class_mapping(gdf, class_mapping)
    code_column = find_column_case_insensitive(gdf.columns, ZWDM_FIELD_NAME)
    if code_column is None:
        raise ValueError(f"分类矢量缺少 {ZWDM_FIELD_NAME} 字段，无法按作物代码生成样方。")

    output = gdf.copy()
    output[ZWDM_FIELD_NAME] = output[code_column].map(normalize_crop_code)
    name_column = find_column_case_insensitive(output.columns, ZWMC_FIELD_NAME)
    code_to_name = crop_name_by_code(crop_code_mapping)
    if name_column is not None:
        output[ZWMC_FIELD_NAME] = output[name_column].map(normalize_mapping_text)
    else:
        output[ZWMC_FIELD_NAME] = output[ZWDM_FIELD_NAME].map(lambda code: code_to_name.get(code, "其它"))
    output[ZWMC_FIELD_NAME] = output[ZWMC_FIELD_NAME].where(
        output[ZWMC_FIELD_NAME].map(bool),
        output[ZWDM_FIELD_NAME].map(lambda code: code_to_name.get(code, "其它")),
    )
    output[CLASS_FIELD_NAME] = output[ZWDM_FIELD_NAME]
    return output
...
def load_classification_gdf(
    ...
    for shp_path in shp_paths:
        ...
        if gdf.empty:
            continue
        gdf = normalize_crop_code_column(gdf, crop_code_mapping, class_mapping)
        gdf = to_metric_crs(gdf, target_crs)
        gdf = keep_polygonal_geometries(gdf)
        if gdf.empty:
            continue
        gdf["src_name"] = shp_path.name
        keep_columns = ["geometry", "src_name", ZWDM_FIELD_NAME, ZWMC_FIELD_NAME, CLASS_FIELD_NAME]
        if ZZLX_FIELD_NAME in gdf.columns:
            keep_columns.append(ZZLX_FIELD_NAME)
        gdf = gdf[[column for column in keep_columns if column in gdf.columns]]
        gdfs.append(gdf)
...
def filter_admin_units_by_classification(
    ...
    for src_name, source_gdf in classification_gdf.groupby("src_name", sort=False):
        source_geom = fast_union(source_gdf.geometry)
        ...
        for unit in admin_units:
            ...
            if not source_geom.intersects(admin_geom):
                continue
            try:
                overlap_area = source_geom.intersection(admin_geom).area
            except Exception:
                overlap_area = source_geom.buffer(0).intersection(admin_geom.buffer(0)).area
            if overlap_area > best_area:
                best_area = overlap_area
                best_unit = unit
...
def infer_class_from_mapping_value(zwmc: object, zwdm: object) -> int | None:
    name = normalize_mapping_text(zwmc)
    code = normalize_mapping_text(zwdm)
    code_digits = re.sub(r"\D", "", code)
    if code_digits in {"107", "108", "1"} or "小麦" in name:
        return CLASS_WHEAT_VALUE
    if code_digits in {"124", "2"} or "油菜" in name:
        return CLASS_RAPE_VALUE
    if code_digits in {"0", "3", "146", "900", "901"} or name:
        return 3
    return None


def infer_class_from_zwmc(value: object) -> int | None:
    name = normalize_mapping_text(value)
    if not name:
        return None
    if "小麦" in name:
        return CLASS_WHEAT_VALUE
...
def normalize_class_column(
    ...
    gdf = apply_flexible_class_mapping(gdf, class_mapping)
    gdf = normalize_class_from_zwmc(gdf)
    class_column = find_class_column(gdf)
    if not class_column:
        raise ValueError(
            f"分类矢量缺少 {CLASS_FIELD_NAME}/{ZWMC_FIELD_NAME} 字段，"
            "无法按地物类别生成样方；也未能按映射 CSV 生成 class。"
        )
    if class_column == CLASS_FIELD_NAME:
        return gdf

    gdf = gdf.copy()
    gdf[CLASS_FIELD_NAME] = gdf[class_column]
    return gdf


def class_numeric_series(gdf: gpd.GeoDataFrame) -> pd.Series:
    if CLASS_FIELD_NAME not in gdf.columns:
        raise ValueError(f"分类矢量缺少 {CLASS_FIELD_NAME} 字段，无法按 class 生成样方。")
    return pd.to_numeric(gdf[CLASS_FIELD_NAME], errors="coerce")
...
def add_sample_record(
    ...
    normalized_code = normalize_crop_code(crop_code if crop_code is not None else sample_class_code)
    records.append(
        {
            ...
            "class": normalized_code,
            "ZWMC": (crop_name or sample_class.replace("样方", ""))[:20],
            "ZWDM": normalized_code[:10],
...
def build_overlap_samples(
    ...
    while (
        ...
        add_sample_record(
            ...
            crop_name=sample_class.replace("样方", ""),
            crop_code=str(sample_class_code),
        )
...
def build_samples(
    ...
    target_codes = set(crop_code_mapping.values())
    code_to_name = crop_name_by_code(crop_code_mapping)
    code_series = county_gdf[ZWDM_FIELD_NAME].map(normalize_crop_code)
    target_gdf = county_gdf[code_series.isin(target_codes)].copy()
    if target_gdf.empty:
        raise ValueError("行政边界内没有匹配到 zuowucode.json 中的 7 种作物图斑。")

    target_all_geom = valid_union(target_gdf, "7 种作物没有有效几何。")
    center_index = CenterDistanceIndex(min_distance)
    records: list[dict[str, object]] = []
    generated_crop_count = 0

    for crop_name, crop_code in crop_code_mapping.items():
        crop_gdf = county_gdf[code_series == crop_code].copy()
        if crop_gdf.empty:
            print(f"WARNING: {county_name} {crop_name}({crop_code}) 没有图斑，无法生成该作物样方。")
            continue
        crop_geom = valid_union(crop_gdf, f"没有可用于生成 {crop_name}({crop_code}) 样方的有效几何。")
        if admin_geom is not None:
            crop_geom = crop_geom.intersection(admin_geom)
            if crop_geom.is_empty:
                print(f"WARNING: {county_name} {crop_name}({crop_code}) 与行政边界没有重叠，跳过。")
                continue
        generated = build_overlap_samples(
            target_geom=crop_geom,
            admin_geom=admin_geom,
            city_name=city_name,
            county_name=county_name,
            source_name=source_name,
            sample_class=f"{crop_name}样方",
            sample_class_code=crop_code,
            rng=rng,
        ...
        if generated > 0:
            generated_crop_count += 1

    # Other samples are generated outside the 7 target crop polygons.
    build_non_overlap_samples(
        all_geom=target_all_geom,
        admin_geom=admin_geom,
        city_name=city_name,
        county_name=county_name,
        source_name=source_name,
        ...
        max_attempts=max_attempts,
        max_no_progress_attempts=max_no_progress_attempts,
        other_search_buffer=other_search_buffer,
        center_index=center_index,
        records=records,
    )
    ...
    return gpd.GeoDataFrame(records, geometry="geometry", crs=county_gdf.crs), generated_crop_count > 0
...
def generate_for_shp(
    ...
    raw_gdf = normalize_crop_code_column(raw_gdf, args.crop_code_mapping, getattr(args, "flex_class_mapping", None))
    ...
    samples_gdf, has_target_crop = build_samples(
        ...
        crop_code_mapping=args.crop_code_mapping,
    )
    ...
    with locked_output(sample_path):
...
_GENERATION_CLASSIFICATION_GDF: gpd.GeoDataFrame | None = None
_GENERATION_OUTPUT_ROOT: Path | None = None
_GENERATION_ARGS: argparse.Namespace | None = None


def initialize_generation_worker(
    classification_gdf: gpd.GeoDataFrame,
    output_root: Path,
    args: argparse.Namespace,
) -> None:
    global _GENERATION_CLASSIFICATION_GDF, _GENERATION_OUTPUT_ROOT, _GENERATION_ARGS
    _GENERATION_CLASSIFICATION_GDF = classification_gdf
    _GENERATION_OUTPUT_ROOT = output_root
    _GENERATION_ARGS = args


def run_generation_task(task: tuple[int, int, dict[str, object], int]) -> dict[str, object]:
    index, total, admin_unit, seed = task
    if _GENERATION_CLASSIFICATION_GDF is None or _GENERATION_OUTPUT_ROOT is None or _GENERATION_ARGS is None:
        raise RuntimeError("并发任务尚未初始化。")
    city_name = str(admin_unit["city"])
    county_name = str(admin_unit["county"])
    unit_start_time = time.perf_counter()
    print(f"[Sample {index}/{total}] Start: {city_name}-{county_name}", flush=True)
    has_target_crop = generate_for_admin_unit(
        admin_unit,
        _GENERATION_CLASSIFICATION_GDF,
        _GENERATION_OUTPUT_ROOT,
        _GENERATION_ARGS,
        random.Random(seed),
    )
    elapsed = time.perf_counter() - unit_start_time
    print(f"[Sample {index}/{total}] Finished in {format_elapsed(elapsed)}", flush=True)
    return {
        "index": index,
        "city": city_name,
        "county": county_name,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="按 00县边界 的 area_name 和空间范围，从 00分类结果 的 ZWDM 作物代码生成区县自检样方。"
    )
    parser.add_argument(
        "--source_root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="7 种作物样方与对应图斑的最小重叠比例，默认 0.2，即样方面积的 20%%。",
    )
    parser.add_argument(
        "--min-overlap-area",
        type=float,
    ...
    parser.add_argument(
        ...
        help="某类样方连续多少次没有新增合格样方后提前停止，适合小区划或图斑很少的情况。",
    )
    parser.add_argument(
        "--other-search-buffer",
        type=float,
        default=DEFAULT_OTHER_SEARCH_BUFFER,
        help="其它样方搜索范围向分类图斑外扩的距离，单位米。",
    )
    parser.add_argument("--only-dir-name", default=None, help="只处理位于指定文件夹名下的 shp，例如 小麦矢量。")
    parser.add_argument(
        "--field-mapping",
        type=Path,
        default=DEFAULT_FIELD_MAPPING_PATH,
        help="可选分类映射 CSV。当前默认优先使用分类矢量 ZWDM 字段；映射 CSV 存在时仍兼容读取。",
    )
    parser.add_argument(
        "--crop-code-json",
        type=Path,
        default=DEFAULT_CROP_CODE_PATH,
        help="7 种作物代码 JSON，格式为 作物名: 作物代码；默认 zuowucode.json。",
    )
    parser.add_argument(
        "--failed-log",
        type=Path,
    ...
    parser.add_argument("--workers", dest="concurrency", type=int, default=argparse.SUPPRESS, help=argparse.SUPPRESS)
...
def main() -> None:
    ...
    args.crop_code_mapping = load_crop_code_mapping(args.crop_code_json)
    ...
    failed_count = 0
    crop_region_count = 0
    crop_regions: list[tuple[int, str]] = []
    ...
    classification_gdf = load_classification_gdf(shp_paths, args.resolved_target_crs, args.crop_code_mapping, args.flex_class_mapping)
    ...
    print(f"Loaded crop codes: {args.crop_code_mapping}")
    ...
    unique_admin_units: list[dict[str, object]] = []
    scheduled_outputs: set[Path] = set()
    for admin_unit in admin_units:
        target_path = sample_output_path(
            output_root,
            str(admin_unit["city"]),
            str(admin_unit["county"]),
        )
        if target_path in scheduled_outputs:
            print(f"[跳过] 同一输出已安排：{target_path}")
            continue
        scheduled_outputs.add(target_path)
        unique_admin_units.append(admin_unit)
    print(f"[准备] 样方任务={len(unique_admin_units)}；并发数={concurrency}")
    tasks = [
        (index, len(unique_admin_units), admin_unit, rng.getrandbits(64))
        for index, admin_unit in enumerate(unique_admin_units, start=1)
    ]
    initialize_generation_worker(classification_gdf, output_root, args)
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        future_to_task = {executor.submit(run_generation_task, task): task for task in tasks}
        for future in concurrent.futures.as_completed(future_to_task):
            task = future_to_task[future]
            index, _, admin_unit, _ = task
            city_name = str(admin_unit["city"])
            county_name = str(admin_unit["county"])
            try:
                info = future.result()
                success_count += 1
                if info["has_target_crop"]:
                    crop_region_count += 1
                    crop_regions.append((index, f"{city_name}-{county_name}"))
    ...
    print(f"Regions with generated target crop samples: {crop_region_count}")
    if crop_regions:
        print("Target crop region list:")
        for _, region in sorted(crop_regions):
            print(f"  {region}")
