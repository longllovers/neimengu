...
FINAL_FIELDS = ("QXMC", "QXDM", "CUNMC", "CUNDM", "YGCUNDM", "ZWMC", "ZWDM", "TBMJ", "CLYX", "BZ")
FINAL_TEXT_FIELDS = ("QXMC", "QXDM", "CUNMC", "CUNDM", "YGCUNDM", "ZWMC", "ZWDM", "BZ")
...
def load_crop_name_by_code(path: Path) -> dict[str, str]:
    fallback = {"900": "其它"}
    ...
    for name, code in data.items():
        crop_name = str(name).strip()
        crop_code = str(code).strip()
        if crop_code.endswith(".0"):
            crop_code = crop_code[:-2]
        if crop_name and crop_code:
            result[crop_code] = crop_name
    result.update(fallback)
    return result


CROP_NAME_BY_CODE = load_crop_name_by_code(DEFAULT_CROP_CODE_PATH)
CROP_CODE_BY_TEXT = tuple((name, code) for code, name in CROP_NAME_BY_CODE.items()) + (("其它", "900"), ("其他", "900"))
CROP_BY_CLASS = {0: ("其它", "900"), 3: ("其它", "900")}
CLYX_VALUE = 0.5
CUNMC_FIELD_CANDIDATES = ("CUNMC", "QHMC", "村名称", "村名", "村社区名")
CUNDM_FIELD_CANDIDATES = ("CUNDM", "QHDM", "村代码", "村级代码", "行政区划代码")
ZWMC_FIELD_CANDIDATES = ("ZWMC", "ZZLX", "sample_cls", "作物名称", "作物类型", "地物类型")
ZWDM_FIELD_CANDIDATES = ("ZWDM", "DWMC", "class_id", "作物代码")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--concurrency", type=int, default=max(1, min((os.cpu_count() or 1), 2)), help="并发数。")
...
def write_final_shp(gdf: gpd.GeoDataFrame, output_path: Path, encoding: str) -> None:
    ...
    with locked_output(output_path):
...
def crop_code_from_text(value: object, fill: bool = False) -> str:
    ...
    if not text:
        return "900"
    code = normalized_code(text, 9)
    if code.isdigit() and code not in {"0"}:
        return code
    for keyword, crop_code in CROP_CODE_BY_TEXT:
        if keyword in text:
            return crop_code
    return text[:9]
...
def crop_name_from_code(code: str) -> str:
    return CROP_NAME_BY_CODE.get(code, "")


def crop_name_from_text(value: object, code: str) -> str:
    known_name = crop_name_from_code(code)
    if known_name:
        return known_name
    text = non_empty_text(value)
    return text.replace("样方", "").strip()
...
def crop_info_from_sample(row) -> tuple[str, str]:
    code_value = first_row_value(row, ZWDM_FIELD_CANDIDATES)
    raw_name = first_row_value(row, ZWMC_FIELD_CANDIDATES) or code_value
    crop_code = normalized_code(code_value, 9)
    if raw_name is not None or crop_code:
        if not crop_code or not crop_code.isdigit():
            crop_code = crop_code_from_text(raw_name)
        crop_name = crop_name_from_text(raw_name, crop_code)
        return crop_name[:20], crop_code[:9]

    if "class" in row.index:
        class_value = class_to_int(row.get("class"))
        if class_value in CROP_BY_CLASS:
            return CROP_BY_CLASS[class_value]

    crop_code = crop_code_from_text(None)
    return crop_name_from_code(crop_code)[:20], crop_code[:9]
...
def reference_code_from_sample(row) -> str:
    ...
    raw_name = first_row_value(row, ZWMC_FIELD_CANDIDATES) or code_value
    crop_code = normalized_code(code_value, 9)
    if crop_code and crop_code.isdigit():
        return crop_code
    if raw_name is not None:
        return crop_code_from_text(raw_name, fill=False)
    if "class" in row.index:
        class_value = class_to_int(row.get("class"))
        if class_value in CROP_BY_CLASS:
            return CROP_BY_CLASS[class_value][1]
    return crop_code_from_text(None, fill=False)


def crop_info_from_result(row) -> tuple[str, str]:
    code_value = first_row_value(row, ZWDM_FIELD_CANDIDATES)
    code = normalized_code(code_value, 9)
    if not code or not code.isdigit():
        code = crop_code_from_text(row.get("ZZLX") if "ZZLX" in row.index else code_value, fill=False)
    name = crop_name_from_code(code)
    if not name:
        name = non_empty_text(row.get("ZZLX") if "ZZLX" in row.index else None)
    return name[:20], code[:9]
...
def delivery_output_path(delivery_dir: Path, county_code: str) -> Path:
    return delivery_dir / county_code / f"KFJDCZYB{county_code}_2026_QL.shp"
...
def build_tasks(args: argparse.Namespace, common_crs: CRS, result_entries: list[ResultEntry]) -> list[ClipTask]:
    ...
    tasks: list[ClipTask] = []
    scheduled_delivery_paths: set[Path] = set()
    for sample_path in sample_paths:
        ...
        if args.mode == "skip" and delivery_path.exists():
            ...
            continue
        if delivery_path in scheduled_delivery_paths:
            print(f"[跳过] 同一输出已安排：{delivery_path}")
            continue
        scheduled_delivery_paths.add(delivery_path)
        tasks.append(
            ClipTask(
...
def read_result_layer(path: Path, bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    if gdf.empty:
        return gpd.GeoDataFrame(columns=["ZZLX", "geometry"], geometry="geometry", crs=target_crs)
    if gdf.crs is None:
    ...
    zwdm_field = find_field(gdf.columns, ("ZWDM", "zwdm", "作物代码", "DWMC", "class_id"), "ZWDM/作物代码")
    keep_fields = [zwdm_field, "geometry"]
    rename_fields = {zwdm_field: "ZWDM"}
    zzlx_field = None
    try:
        zzlx_field = find_field(gdf.columns, ("ZWMC", "ZZLX", "zzlx", "作物名称", "作物类型", "地物类型"), "ZWMC/ZZLX")
    except ValueError:
        zzlx_field = None
    if zzlx_field is not None and zzlx_field not in keep_fields:
        keep_fields.append(zzlx_field)
        rename_fields[zzlx_field] = "ZZLX"
    gdf = gdf[keep_fields].rename(columns=rename_fields).copy()
    if "ZZLX" not in gdf.columns:
        gdf["ZZLX"] = gdf["ZWDM"]
    gdf["geometry"] = gdf.geometry.apply(polygonal_only)
    gdf = gdf[gdf.geometry.notna() & ~gdf.geometry.is_empty].copy()
    return gdf
...
def load_results(result_paths: Iterable[str], bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    for path_text in result_paths:
        ...
        if not part.empty:
            ...
            parts.append(part)
    if not parts:
        return gpd.GeoDataFrame(columns=["ZZLX", "src_file", "geometry"], geometry="geometry", crs=target_crs)
    ...
    for sample_index, sample_row in sample.iterrows():
        ...
        if fill_geom is not None and fill_geom.area > task.area_epsilon:
            ...
            for part in iter_polygon_parts(fill_geom):
                if part.area <= task.area_epsilon:
                    continue
                rows.append(make_output_row(task.qxmc, task.qxdm, sample_row, part, fill_crop))
...
def main() -> int:
    ...
    print(f"[准备] 结果 shp={len(result_entries)}；样方任务={len(tasks)}；并发数={concurrency}")
    success_count = 0
