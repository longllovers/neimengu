...
class JobController:
    ...
    def start(self, form: dict[str, str]) -> tuple[bool, str]:
        with self._lock:
            ...
            return True, f"任务已启动，PID={process.pid}"
    ...
    def stop(self) -> tuple[bool, str]:
        with self._lock:
            ...
            if self.stopping:
                return True, f"任务正在停止，PID={process.pid}，请稍候。"
            self.stopping = True
            ...
            return True, (
                f"停止请求已接收，正在温和停止并清理整个任务并发数组，PID={process.pid}；"
                "最多等待 8 秒。"
            )

    def _terminate_process_tree(self, process: subprocess.Popen[str]) -> None:
        """先温和停止整个任务并发数组，8 秒未退出再强制结束。"""
        pid = process.pid
        forced = False
        error_message = ""
        try:
            ...
            if error_message:
                line = f"停止并发数组时出现异常：{error_message}"
                level = "error"
            elif forced:
                line = f"任务在 8 秒内未退出，已强制结束整个并发数组，PID={pid}"
                level = "warning"
            else:
                line = f"任务已完成清理并停止，PID={pid}"
                level = "warning"
...
<main>
  <form class="panel controls" method="post" action="/start">
    ...
    <div class="grid">
      ...
      <div class="field full"><label>GDAL bin 目录（需含 warp/buildvrt/addo，通常可空）</label>
  ...
  <section class="workspace">
    ...
    <div class="panel table-panel">
      <div class="panel-head"><h2>逐县处理状态</h2><span class="stage">每个县固定一行，状态原位更新</span></div>
...
(() => {{
  ...
  stopButton.addEventListener("click", async () => {{
    ...
    document.getElementById("stageText").textContent = "正在结束裁剪并发数组";
    try {{
