...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "将畜禽养殖场 Excel 属性表连接到 Shapefile：FCBM 有值时按"
            "序号+分厂编码组合连接，FCBM 为空时按序号连接，并输出标准字段 Shapefile。"
        )
    )
...
def expand_merged_excel_cells(excel_path, sheet_name, raw):
    """
    将数据区的合并单元格展开到合并范围内的每一行。

    pandas 读取 Excel 时只保留合并区域左上角的值。若序号、养殖场名称、
    养殖场编码等字段纵向合并，按“序号 + 分厂编码”选中后续分厂行时，
    这些属性就会变成空值。这里依据 Excel 文件本身的合并范围精确补值，
    不对普通空单元格做无差别向下填充。
    """
    if raw.empty:
        return raw
    ...
    if suffix == ".xls":
        ...
        workbook = xlrd.open_workbook(str(excel_path), formatting_info=True)
        sheet = workbook.sheet_by_name(sheet_name)
        # xlrd: (row_start, row_end, col_start, col_end)，结束位置不包含。
        ...
        # openpyxl 使用从 1 开始且包含结束位置的坐标，统一转成切片坐标。
        merged_ranges = [
            (cell_range.min_row - 1, cell_range.max_row,
             cell_range.min_col - 1, cell_range.max_col)
    ...
    for row_start, row_end, col_start, col_end in merged_ranges:
        # 只展开表头之后的数据区，避免改变多层表头识别结果。
        if row_start <= header_row:
            continue
        if row_start >= len(expanded) or col_start >= len(expanded.columns):
            continue

        bounded_row_end = min(row_end, len(expanded))
        bounded_col_end = min(col_end, len(expanded.columns))
        value = expanded.iat[row_start, col_start]
        expanded.iloc[
            row_start:bounded_row_end,
            col_start:bounded_col_end,
        ] = value
...
def read_excel_with_auto_header(excel_path):
    ...
    for sheet_name in excel_file.sheet_names:
        ...
        candidate = expand_merged_excel_cells(excel_path, sheet_name, candidate)
...
def normalize_join_id(series):
    ...
    result = result.replace({"nan": "", "None": "", "none": ""})
    return result


def normalize_fcbm(series):
    """
    统一分厂编码格式，便于 Shapefile 的 FCBM 与 Excel 的“分厂编码”匹配。
    例如：1、1.0、'0001' 都统一成 '0001'；空值统一成空字符串。
    """
    import pandas as pd
    ...
    result = result.str.replace("\u00A0", "", regex=False)
    result = result.str.replace(r"\s+", "", regex=True)
    result = result.replace({"nan": "", "NaN": "", "None": "", "none": ""})

    numeric = pd.to_numeric(result, errors="coerce")
    integer_numeric = numeric.notna() & (numeric % 1 == 0)
    result.loc[integer_numeric] = numeric.loc[integer_numeric].astype(int).astype(str)

    non_blank = result != ""
    result.loc[non_blank] = result.loc[non_blank].str.zfill(4)
    return result
...
def build_extract_data(df, debug=False):
    ...
    excel_fcbm_col = get_col_name(df, FIELD_MAPPING["FCBM"])
    raw_id_numeric = pd.to_numeric(df[id_col], errors="coerce")
    # Excel 的“序号”可能纵向合并。pandas 只会读到合并区域首行，
    # 后续分厂行为空，因此先向下补齐序号，再与分厂编码组成匹配键。
    temp_numeric = raw_id_numeric.ffill()
    normalized_excel_fcbm = (
        normalize_fcbm(df[excel_fcbm_col])
        if excel_fcbm_col is not None
        else pd.Series("", index=df.index, dtype=str)
    )
    # 只保留序号原始行，以及带分厂编码的合并续行，避免把表尾备注等
    # 普通空序号行错误继承为上一条记录。
    valid_rows = temp_numeric.notna() & (
        raw_id_numeric.notna() | (normalized_excel_fcbm != "")
    )
    df = df[valid_rows].copy()

    if df.empty:
        raise ValueError("Excel 未找到有效序号或分厂编码。")
    ...
    extract_data["FCBM"] = normalize_fcbm(extract_data["FCBM"])
...
def update_shapefile(
    ...
    actual_shp_fcbm_field = resolve_shp_field(gdf.columns, "FCBM")
    if actual_shp_fcbm_field is not None:
        shp_fcbm = normalize_fcbm(gdf[actual_shp_fcbm_field])
    else:
        shp_fcbm = gdf["序号"].map(lambda _: "")

    # FCBM 有值时按“xh + FCBM”组合匹配；FCBM 为空时只按 xh/序号匹配。
    use_fcbm = shp_fcbm != ""
    gdf["__join_key"] = "xh:" + gdf["序号"]
    gdf.loc[use_fcbm, "__join_key"] = (
        "xh_fcbm:"
        + gdf.loc[use_fcbm, "序号"]
        + ":"
        + shp_fcbm.loc[use_fcbm]
    )

    # 只按 xh 时，每个序号只取 Excel 的第一行，避免合并序号下有多个分厂行时
    # 一个 Shapefile 图斑被重复展开；有 FCBM 时则由下面的组合键精确选行。
    excel_by_xh = (
        extract_data.loc[extract_data["序号"] != ""]
        .drop_duplicates(subset=["序号"], keep="first")
        .copy()
    )
    excel_by_xh["__join_key"] = "xh:" + excel_by_xh["序号"]
    excel_by_fcbm = extract_data.loc[extract_data["FCBM"] != ""].copy()
    excel_by_fcbm["__join_key"] = (
        "xh_fcbm:"
        + excel_by_fcbm["序号"]
        + ":"
        + excel_by_fcbm["FCBM"]
    )
    excel_lookup = pd.concat(
        [excel_by_fcbm, excel_by_xh],
        ignore_index=True,
    )
    ...
    if debug:
        ...
        print("\n==== xh + FCBM 组合匹配诊断 ====")
        print(f"SHP存在FCBM字段: {actual_shp_fcbm_field is not None}")
        print(f"按xh+FCBM匹配的SHP要素数: {int(use_fcbm.sum())}")
        print(f"只按xh匹配的SHP要素数: {int((~use_fcbm).sum())}")
        print(f"Excel有效分厂编码数: {int((extract_data['FCBM'] != '').sum())}")
    ...
    if final_gdf.empty:
        raise ValueError(
            "输出结果为空，请检查 Shapefile 的 FCBM/xh 与 "
            "Excel 的分厂编码/序号是否匹配。"
        )
