...
def county_task_from_payload(payload: dict[str, object]) -> tuple[CountyTask, RuntimeConfig]:
    ...
    if not isinstance(task_data, dict) or not isinstance(config_data, dict):
        raise ValueError("县级子并发数任务文件格式无效")
    task = CountyTask(
    ...
    if not isinstance(excluded, list):
        raise ValueError("县级子并发数排除路径格式无效")
...
def county_worker_main(task_file: Path) -> int:
    """在独立 Python 并发数中执行一个县，结果通过标准输出返回给父并发数。"""
    ...
    try:
        ...
        LOG.exception("县级子并发数处理失败：%s", exc)
...
def log_worker_line(task: CountyTask, line: str) -> None:
    prefix = f"[{task.code} {task.name}] 子并发数："
...
def process_county_subprocess(
    ...
    try:
        ...
        if result is None:
            raise RuntimeError(f"县级子并发数异常退出（退出码 {exit_code}），未返回处理结果")
        error = str(result.get("error", ""))
        status = str(result.get("status", "failed"))
        if exit_code != 0 or status == "failed":
            raise RuntimeError(error or f"县级子并发数退出码 {exit_code}")
        if status not in {"success", "skipped", "no_data"}:
            raise RuntimeError(f"县级子并发数返回未知状态：{status}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if len(set(normalized_outputs)) != len(normalized_outputs):
            raise ValueError("输出文件名存在重复；不同县不能由子并发数同时写入同一文件")
        ...
        LOG.info(
            "资源计划：逻辑 CPU=%d，CPU=%.1f%%，请求并发=%d，县级子并发数=%d，索引线程=%d",
        ...
        config = RuntimeConfig(
            ...
            # 任何县级子并发数的输入源，避免一个并发数写、另一个并发数读取。
        ...
        # 子并发数中运行，隔离 GDAL 状态和内存，并保留受控的最大并发数。
