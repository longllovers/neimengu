...
FINAL_FIELDS = ("QXMC", "QXDM", "CLTBLXDM", "CKDWMJ", "CLDWLX", "CLDWMJ")
FINAL_SCHEMA = {
    "geometry": "Polygon",
    "properties": {
        "QXMC": "str:30",
...
def read_result_layer(path: Path, bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    if gdf.empty:
        return gpd.GeoDataFrame(columns=["TBLXDM", "geometry"], geometry="geometry", crs=target_crs)
    if gdf.crs is None:
    ...
    gdf = gdf[[tblxdm_field, "geometry"]].rename(columns={tblxdm_field: "TBLXDM"}).copy()
    gdf["geometry"] = gdf.geometry.apply(polygonal_only)
    gdf = gdf[gdf.geometry.notna() & ~gdf.geometry.is_empty].copy()
    return gdf
...
def load_results(result_paths: Iterable[str], bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    for path_text in result_paths:
        ...
        if not part.empty:
            ...
            parts.append(part)
    if not parts:
        return gpd.GeoDataFrame(columns=["TBLXDM", "src_file", "geometry"], geometry="geometry", crs=target_crs)
...
def make_output_row(qxmc: str, qxdm: str, reference_code: str, measure_code: str, geom) -> dict[str, object]:
    ...
    reference_code = crop_code_from_text(reference_code)
    measure_code = reference_code
    return {
        "QXMC": qxmc,
        "QXDM": qxdm,
        "CLTBLXDM": reference_code,
        "CKDWMJ": area,
        "CLDWLX": measure_code,
        "CLDWMJ": area,
        "geometry": geom,
...
def clip_samples_to_results(sample: gpd.GeoDataFrame, results: gpd.GeoDataFrame, task: ClipTask) -> gpd.GeoDataFrame:
        ...
        if spatial_index is not None:
            ...
            for result_index in list(candidate_indices):
                ...
                for part in iter_polygon_parts(clipped_geom):
                    ...
                    rows.append(make_output_row(task.qxmc, task.qxdm, reference_code, reference_code, part))
    ...
    output["CLDWLX"] = output["CLTBLXDM"]
    for field in ("QXMC", "QXDM", "CLTBLXDM", "CLDWLX"):
