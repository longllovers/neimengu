...
主并发数只负责扫描、调度和汇总。每个文件、每个阶段都由一个一次性的独立
Python 子并发数处理；同一子并发数不会继续处理下一个文件。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    if args.worker and (not args.stage or not args.file):
        parser.error("子并发数缺少 --stage 或 --file")
    return args
...
def run_stage(paths: list[Path], stage: str, args: argparse.Namespace) -> Iterator[Result]:
    ...
    while running:
        ...
        try:
            ...
            detail = stderr.strip() or stdout.strip() or f"子并发数异常退出（代码 {process.returncode}）"
