...
def infer_class_from_mapping_value(zwmc: object, zwdm: object) -> int | None:
    ...
    if code_digits in {"0", "3", "146", "901"} or name:
        return 3
    return None
...
def build_samples(
    ...
    all_geom = valid_union(county_gdf, "输入 shp 没有有效几何。")
    class_values = class_numeric_series(county_gdf)
    wheat_gdf = county_gdf[class_values == CLASS_WHEAT_VALUE].copy()
    generate_rape = should_generate_rape_samples(county_name)
    rape_gdf = county_gdf[class_values == CLASS_RAPE_VALUE].copy() if generate_rape else county_gdf.iloc[0:0].copy()
    has_rape = generate_rape and not rape_gdf.empty

    center_index = CenterDistanceIndex(min_distance)
    records: list[dict[str, object]] = []

    if not wheat_gdf.empty:
        wheat_sample_class = crop_sample_class_from_gdf(wheat_gdf, CLASS_WHEAT)
        wheat_geom = valid_union(wheat_gdf, "没有可用于生成小麦样方的有效几何。")
        if admin_geom is not None:
            wheat_geom = wheat_geom.intersection(admin_geom)
            if wheat_geom.is_empty:
                raise ValueError("小麦图斑与行政边界没有重叠，无法生成小麦样方。")
        build_overlap_samples(
            target_geom=wheat_geom,
            admin_geom=admin_geom,
            city_name=city_name,
            county_name=county_name,
            source_name=source_name,
            sample_class=wheat_sample_class,
            sample_class_code=CLASS_WHEAT_VALUE,
            rng=rng,
    ...
    if not rape_gdf.empty:
        rape_sample_class = crop_sample_class_from_gdf(rape_gdf, CLASS_RAPE)
        rape_geom = valid_union(rape_gdf, "没有可用于生成油菜样方的有效几何。")
        if admin_geom is not None:
            rape_geom = rape_geom.intersection(admin_geom)
            if rape_geom.is_empty:
                raise ValueError("油菜图斑与行政边界没有重叠，无法生成油菜样方。")
        build_overlap_samples(
            target_geom=rape_geom,
            admin_geom=admin_geom,
            city_name=city_name,
            county_name=county_name,
            source_name=source_name,
            sample_class=rape_sample_class,
            sample_class_code=CLASS_RAPE_VALUE,
            rng=rng,
            sample_count=sample_count,
            min_distance=min_distance,
            square_size=square_size,
    ...
    # Other samples are generated from blank areas: the whole sample square must not
    # intersect any input result polygon, regardless of the polygon's ZZLX/class value.
    ...
    return gpd.GeoDataFrame(records, geometry="geometry", crs=county_gdf.crs), has_rape
...
def generate_for_admin_unit(
    ...
    print(f"[处理行政区] {city_name}-{county_name}")
...
def stable_unit_seed(base_seed: int, city_name: str, county_name: str) -> int:
    """为每个区县生成稳定随机种子，使并发执行仍可复现。"""
    digest = hashlib.sha256(f"{base_seed}|{city_name}|{county_name}".encode("utf-8")).digest()
    return int.from_bytes(digest[:8], "big", signed=False)


def run_admin_unit_task(
    index: int,
    total: int,
    admin_unit: dict[str, object],
    county_gdf: gpd.GeoDataFrame,
    ...
    print(f"[样方 {index}/{total}] 开始：{city_name}-{county_name}")
    rng = random.Random(stable_unit_seed(args.seed, city_name, county_name))
    has_rape = generate_for_admin_unit(admin_unit, county_gdf, output_root, args, rng)
    return {
        "index": index,
        "city": city_name,
        "county": county_name,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="小麦/油菜样方与对应图斑的最小重叠比例，默认 0.2，即样方面积的 20%%。",
    )
    parser.add_argument(
        "--min-overlap-area",
        type=float,
...
def main() -> None:
    ...
    print(f"开始按新行政边界生成样方；并发数={concurrency}。")

    tasks: list[tuple[int, dict[str, object], gpd.GeoDataFrame]] = []
    seen_outputs: set[str] = set()
    total_units = len(admin_units)
    for index, admin_unit in enumerate(admin_units, start=1):
        city_name = str(admin_unit["city"])
        county_name = str(admin_unit["county"])
        output_key = str(sample_output_path(output_root, city_name, county_name).resolve()).casefold()
        if output_key in seen_outputs:
            print(f"[跳过] 输出路径重复：{city_name}-{county_name}")
            continue
        ...
        if args.mode == "skip" and sample_path.exists():
            ...
            success_count += 1
            continue
        county_gdf = select_classification_for_admin(classification_gdf, admin_unit["geometry"])
        if county_gdf.empty:
            print(f"FAILED: {city_name}-{county_name} - 新行政边界内没有匹配到任何分类图斑。")
            failed_count += 1
            continue
        tasks.append((index, admin_unit, county_gdf))

    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        future_to_name = {
            executor.submit(run_admin_unit_task, index, total_units, admin_unit, county_gdf, output_root, args): (
                str(admin_unit["city"]),
                str(admin_unit["county"]),
            )
            for index, admin_unit, county_gdf in tasks
        }
        for future in concurrent.futures.as_completed(future_to_name):
            city_name, county_name = future_to_name[future]
            try:
                info = future.result()
                success_count += 1
                print(
                    f"[样方 {info['index']}/{total_units}] 完成：{city_name}-{county_name}；"
                    f"用时={format_elapsed(float(info['elapsed']))}"
                )
                if info["has_rape"]:
                    rape_count += 1
    ...
    print(f"ZZLX has rape regions: {rape_count}")
    if rape_regions:
