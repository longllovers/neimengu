...
FINAL_FIELDS = (
    "QXMC", "QXDM",
    "CLTBLXMC", "CLTBLXDM", "CLTBEJLMC", "CLTBEJLDM", "CLTBMJ",
    "CKTBLXMC", "CKTBLXDM", "CKTBEJLMC", "CKTBEJLDM", "CKTBMJ",
    "BZHU",
)
FINAL_SCHEMA = {
    "geometry": "Polygon",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("overwrite", "skip"), default="overwrite", help="skip 跳过已有参考真值；overwrite 重新裁剪并覆盖。")
...
TEXT_CODE_FIELDS = ("TBLXDM", "ZWDM", "CLTBLXDM", "CLDWLX")
TEXT_NAME_FIELDS = ("sample_cls",)
...
def crop_code_from_text(value: object, fill: bool = False) -> str:
    ...
    if text.endswith(".0"):
        text = text[:-2]
    digits = re.sub(r"\D", "", text)
    return digits if digits else text[:10]

def reference_code_from_sample(row) -> str:
    return crop_code_from_text(first_row_value(row, TEXT_CODE_FIELDS), fill=False)
...
def build_tasks(args: argparse.Namespace, common_crs: CRS, result_entries: list[ResultEntry]) -> list[ClipTask]:
    print(f"[02] 开始构建裁剪任务：result_entries={len(result_entries)}", flush=True)
    ...
    print(f"[02] 找到样方文件数：{len(sample_paths)}；样方目录={args.sample_dir}", flush=True)
    if not sample_paths:
    ...
    for sample_path in sample_paths:
        ...
        if not sample_matches_filter(city, county, args.only_boundary_name):
            continue
        print(f"[02] 准备样方任务：{city}-{county}，文件={sample_path.name}", flush=True)
        unit = resolve_boundary_unit(city, county, by_city_name, by_name)
        county_entries = result_entries_by_county.get(unit.code, [])
        print(f"[02] 匹配边界：{unit.name}-{unit.code}；候选结果文件数={len(county_entries)}", flush=True)
        ...
        if args.mode == "skip" and delivery_path.exists():
            ...
            continue
        print(f"[02] 新增裁剪任务：输出={delivery_path}", flush=True)
        tasks.append(
            ClipTask(
    ...
    print(f"[02] 裁剪任务构建完成：tasks={len(tasks)}", flush=True)
    return tasks
...
def read_result_layer(path: Path, bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    if gdf.empty:
        return gpd.GeoDataFrame(columns=["TBEJLDM", "geometry"], geometry="geometry", crs=target_crs)
    if gdf.crs is None:
    ...
    tbejl_field = find_field(gdf.columns, ("TBEJLDM",), "TBEJLDM")
    if tbejl_field != "TBEJLDM":
        gdf = gdf.rename(columns={tbejl_field: "TBEJLDM"})
    gdf["TBEJLDM"] = gdf["TBEJLDM"].map(crop_code_from_text)
    gdf["geometry"] = gdf.geometry.apply(polygonal_only)
    gdf = gdf[gdf.geometry.notna() & ~gdf.geometry.is_empty].copy()
    return gdf
...
def load_results(result_paths: Iterable[str], bbox_value: tuple[float, float, float, float], target_crs, input_encoding: str | None) -> gpd.GeoDataFrame:
    ...
    for path_text in result_paths:
        ...
        if not part.empty:
            ...
            parts.append(part)
    if not parts:
        return gpd.GeoDataFrame(columns=["TBEJLDM", "src_file", "geometry"], geometry="geometry", crs=target_crs)
    return gpd.GeoDataFrame(pd.concat(parts, ignore_index=True, sort=False), geometry="geometry", crs=parts[0].crs)


def make_output_row(qxmc: str, qxdm: str, result_row, geom) -> dict[str, object]:
    area = round(float(geom.area))
    primary_name = non_empty_text(result_row.get("TBLXMC"))[:16] or "农作物"
    primary_code = crop_code_from_text(result_row.get("TBLXDM"))[:4] or "100"
    secondary_name = non_empty_text(result_row.get("TBEJLMC"))[:16] or "大棚"
    remark = non_empty_text(result_row.get("BZHU"))[:20]
    return {
        "QXMC": qxmc,
        "QXDM": qxdm,
        "CLTBLXMC": primary_name,
        "CLTBLXDM": primary_code,
        "CLTBEJLMC": secondary_name,
        "CLTBEJLDM": GREENHOUSE_CODE,
        "CLTBMJ": area,
        "CKTBLXMC": primary_name,
        "CKTBLXDM": primary_code,
        "CKTBEJLMC": secondary_name,
        "CKTBEJLDM": GREENHOUSE_CODE,
        "CKTBMJ": area,
        "BZHU": remark,
        "geometry": geom,
    }
...
def clip_samples_to_results(sample: gpd.GeoDataFrame, results: gpd.GeoDataFrame, task: ClipTask) -> gpd.GeoDataFrame:
            ...
            for result_index in list(candidate_indices):
                ...
                for part in iter_polygon_parts(clipped_geom):
                    if part.area <= task.area_epsilon:
                        continue
                    rows.append(make_output_row(task.qxmc, task.qxdm, result_row, part))
    ...
    for field in FINAL_FIELDS:
        if field in ("CLTBMJ", "CKTBMJ"):
            continue
        output[field] = output[field].astype(str)
    for field in ("CLTBMJ", "CKTBMJ"):
...
def process_task(task: ClipTask) -> dict[str, object]:
    print(f"[02] 开始处理任务：{task.city}-{task.county} -> {task.delivery_path}", flush=True)
    warnings.filterwarnings("ignore", category=UserWarning)
    sample_path = Path(task.sample_path)
    sample = read_sample(sample_path)
    print(f"[02] 样方读取完成：{sample_path.name}，features={len(sample)}", flush=True)
    ...
    print(f"[02] 裁剪输出要素数：{len(output)}", flush=True)
    if output.empty:
...
def main() -> int:
    args = parse_args()
    print(f"[02] result_root={args.result_root}；sample_dir={args.sample_dir}；output_root={args.output_root}", flush=True)
    ...
    print(f"[02] 发现结果 shp 数：{len(result_shps)}", flush=True)
    if not result_shps:
