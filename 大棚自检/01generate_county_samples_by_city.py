...
def get_admin_city_name(row: object, source_path: Path, fallback_city_name: str | None) -> str:
    ...
    if code_value:
        city_name = CITY_CODE_TO_NAME.get(code_value[:4])
        if city_name:
            return city_name
...
def load_classification_gdf(
    ...
    for shp_index, shp_path in enumerate(shp_paths, start=1):
        ...
        if gdf.empty:
            continue
        gdf = normalize_greenhouse_column(gdf)
        greenhouse_count = int((gdf[GREENHOUSE_FIELD_NAME] == GREENHOUSE_CODE).sum())
        print(f"[01] {GREENHOUSE_FIELD_NAME}={GREENHOUSE_CODE} 大棚要素数：{greenhouse_count}", flush=True)
        ...
        if GREENHOUSE_FIELD_NAME in gdf.columns:
            keep_columns.append(GREENHOUSE_FIELD_NAME)
...
def normalize_greenhouse_column(gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    greenhouse_column = find_column_case_insensitive(gdf.columns, GREENHOUSE_FIELD_NAME)
    if greenhouse_column is None:
        raise ValueError(f"分类矢量缺少 {GREENHOUSE_FIELD_NAME} 字段，无法识别大棚。")
    output = gdf.copy()
    output[GREENHOUSE_FIELD_NAME] = output[greenhouse_column].map(crop_code_text)
    return output
...
def build_samples(
    ...
    if GREENHOUSE_FIELD_NAME not in county_gdf.columns:
        raise ValueError(f"分类矢量缺少 {GREENHOUSE_FIELD_NAME} 字段，无法识别大棚。")
    if sample_count <= 0 or square_size <= 0:
        raise ValueError("样方数量和网格边长必须大于 0。")

    sampling_geom = extract_polygonal_geometry(admin_geom)
    if sampling_geom is None:
        sampling_geom = extract_polygonal_geometry(fast_union(county_gdf.geometry))
    if sampling_geom is None:
        raise ValueError("没有可用于随机抽样的有效行政区范围。")

    greenhouse_gdf = county_gdf[
        county_gdf[GREENHOUSE_FIELD_NAME].map(crop_code_text) == GREENHOUSE_CODE
    ].copy()
    greenhouse_gdf = keep_polygonal_geometries(greenhouse_gdf).reset_index(drop=True)
    has_greenhouse = not greenhouse_gdf.empty
    greenhouse_index = greenhouse_gdf.sindex if has_greenhouse else None
    print(
        f"[01] {county_name} 大棚状态：{'存在' if has_greenhouse else '不存在'}；"
        f"{GREENHOUSE_FIELD_NAME}={GREENHOUSE_CODE} 要素数={len(greenhouse_gdf)}",
        flush=True,
    )

    prepared_sampling_geom = prep(sampling_geom)
    minx, miny, maxx, maxy = sampling_geom.bounds
    half_size = square_size / 2.0
    if maxx - minx < square_size or maxy - miny < square_size:
        raise ValueError(f"行政区范围不足以容纳 {square_size:.0f}m × {square_size:.0f}m 网格。")

    attempts = 0
    rejected_groups = 0
    accepted_group: list[tuple[float, float, object, float]] | None = None
    while attempts < max_attempts:
        group: list[tuple[float, float, object, float]] = []
        while len(group) < sample_count and attempts < max_attempts:
            attempts += 1
            x = rng.uniform(minx + half_size, maxx - half_size)
            y = rng.uniform(miny + half_size, maxy - half_size)
            sample_square = box(x - half_size, y - half_size, x + half_size, y + half_size)
            if not square_inside_prepared_admin(sample_square, prepared_sampling_geom):
                continue
            if any(math.hypot(x - old_x, y - old_y) < min_distance for old_x, old_y, _, _ in group):
                continue

            greenhouse_overlap = 0.0
            if has_greenhouse and greenhouse_index is not None:
                try:
                    candidate_indices = list(greenhouse_index.query(sample_square, predicate="intersects"))
                except Exception:
                    candidate_indices = list(greenhouse_gdf[greenhouse_gdf.intersects(sample_square)].index)
                for candidate_index in candidate_indices:
                    greenhouse_geom = extract_polygonal_geometry(
                        greenhouse_gdf.iloc[int(candidate_index)].geometry
                    )
                    if greenhouse_geom is not None:
                        greenhouse_overlap += float(sample_square.intersection(greenhouse_geom).area)
            group.append((x, y, sample_square, greenhouse_overlap))

        if len(group) < sample_count:
            break
        if has_greenhouse and not any(overlap > 0 for _, _, _, overlap in group):
            rejected_groups += 1
            if rejected_groups % 100 == 0:
                print(
                    f"[01] {county_name} 已重抽 {rejected_groups} 组：每组均未抽中大棚。",
                    flush=True,
                )
            continue
    ...
    if accepted_group is None:
        greenhouse_hint = "且至少一个网格抽中大棚" if has_greenhouse else ""
        raise ValueError(
            f"在 {max_attempts} 次随机尝试内无法生成 {sample_count} 个完整位于县界内的 "
            f"{square_size:.0f}m × {square_size:.0f}m 网格{greenhouse_hint}。"
        )

    records: list[dict[str, object]] = []
    for sample_id, (x, y, sample_square, greenhouse_overlap) in enumerate(accepted_group, start=1):
        hits_greenhouse = greenhouse_overlap > 0
        sample_code = GREENHOUSE_CODE if hits_greenhouse else ""
        sample_name = GREENHOUSE_NAME if hits_greenhouse else "非大棚"
        records.append(
            {
                "sample_id": sample_id,
                "city": city_name,
                "county": county_name,
                "sample_cls": sample_name,
                "class": sample_code,
                TBLXDM_FIELD_NAME: sample_code,
                TBLXMC_FIELD_NAME: sample_name,
                GREENHOUSE_FIELD_NAME: sample_code,
                "overlap": round(greenhouse_overlap, 3),
                "center_x": float(x),
                "center_y": float(y),
                "src_name": source_name[:80],
                "geometry": sample_square,
    ...
    greenhouse_grid_count = sum(record[GREENHOUSE_FIELD_NAME] == GREENHOUSE_CODE for record in records)
    print(
        f"[01] {county_name} 已生成 {len(records)} 个 {square_size:.0f}m × {square_size:.0f}m 检验网格；"
        f"抽中大棚网格数={greenhouse_grid_count}；重抽组数={rejected_groups}；随机尝试数={attempts}",
        flush=True,
    )
...
def generate_for_shp(
    ...
    raw_gdf = normalize_greenhouse_column(raw_gdf)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="按县级行政边界随机生成 3 个 2km×2km 大棚检验网格；TBEJLDM=801 代表大棚。"
    )
    parser.add_argument(
        "--source_root",
        type=Path,
    ...
    parser.add_argument("--sample-count", type=int, default=DEFAULT_SAMPLE_COUNT, help="每个区县生成的检验网格数量，默认 3。")
    parser.add_argument("--min-distance", type=float, default=DEFAULT_MIN_DISTANCE, help="样点最小间距，单位米。")
    parser.add_argument("--square-size", type=float, default=DEFAULT_SQUARE_SIZE, help="检验网格边长，默认 2000 米。")
    parser.add_argument(
        "--min-overlap-ratio",
        type=float,
    ...
    parser.add_argument("--mode", choices=("skip", "overwrite"), default="overwrite", help="skip 跳过已有样方；overwrite 重新生成并覆盖。")
