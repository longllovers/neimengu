...
def stream_command(command: list[str], title: str) -> tuple[int, Path | None]:
    """实时转发子并发数日志，并提取 merge 脚本给出的唯一市级 TIFF。"""
...
def download_groups_concurrently(
    ...
    console.print(
        f"[blue]  TIFF 抽取队列：{tif_workers} 个独立转换线程；"
        "转换期间下载槽继续工作[/blue]"
    )
    ...
    try:
        while pending:
            ...
            tif_executor = ThreadPoolExecutor(
                max_workers=tif_workers,
                thread_name_prefix="tif-extract",
            )
            try:
                download_futures = {}
                for (
                ...
                while download_futures or tif_futures:
                        ...
                        set(download_futures) | set(tif_futures),
                        ...
                        if future in download_futures:
                            ...
                            try:
                                ...
                                console.print(
                                    ...
                                    f"第 {item[1]}/{len(group.products)} 景下载线程异常："
                                    f"{exc}[/bold red]"
                                )
