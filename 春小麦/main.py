...
def page_html(message=""):
    ...
    any_running = run1["running"] or run2["running"]
    run1_disabled = "disabled" if any_running else ""
    run2_disabled = "disabled" if any_running else ""
    ...
    <div class="grid">
        ...
        <section class="card">
            ...
            <form id="run2-form" method="post" action="/run2">
                <label for="run2_source_root">Source Root <span>与运行-1相同</span></label>
                ...
                <input id="input_root" name="input_root" value="{input_root}" placeholder="运行-1后自动填写，也可手动输入包含 01、02 文件夹的根路径" required>
...
function updateStatus(prefix, job, anyRunning) {{
    ...
    if (button) button.disabled = Boolean(anyRunning);
}}
...
async function refreshStatus() {{
    try {{
        ...
        const anyRunning = Boolean(data.run1.running || data.run2.running);
        updateStatus("run1", data.run1, anyRunning);
        updateStatus("run2", data.run2, anyRunning);
            ...
            with JOBS_LOCK:
                ...
                if not busy:
                    ...
                    JOBS["run2"].update(
                        ...
                        returncode=None, output="", csv_output="", source_root="",
