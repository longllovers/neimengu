...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--merge-small-output",
        type=Path,
        help="可选。先合并碎小图斑，并把合并后的 Shapefile 写到该路径。",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="允许覆盖已有 CSV 和合并后的 Shapefile。",
    )
...
def main() -> int:
    ...
    try:
        ...
        output_path = args.output or Path("CSV") / f"{input_path.stem}.csv"
        existing_outputs = [path for path in (output_path, args.merge_small_output) if path is not None and path.exists()]
        if existing_outputs and not args.overwrite:
            paths = "、".join(str(path) for path in existing_outputs)
            raise FileExistsError(f"输出已存在：{paths}；如需覆盖请启用覆盖选项。")
        check_input = input_path
        if args.merge_small_output is not None:
            merge_small_features(
                input_path,
                args.merge_small_output,
                id_field=args.id_field,
                min_mu=args.min_mu,
            )
            check_input = args.merge_small_output
        check_shp(
            check_input,
            output_path,
            args.id_field,
